...
import pygame
pygame.init()
img = pygame.image.load("士兵.png")
img = pygame.image.load("火人.png")
img = pygame.image.load("死神.png")
img = pygame.image.load("哥布林.png")
img = pygame.image.load("黑影.png")
img = pygame.image.load("魔像.png")



# 通用槍枝音效，用來綁定不同武器類型的專屬聲音拿來當備用，如果專用槍枝音效出問題了就會退回這個通用槍聲，如果我程式沒寫錯的話
...
try:
    ...
    pygame.mixer.music.set_volume(0.2) # BGM 音量需要小一點不要蓋過槍聲
except:
...
WEAPON_TYPES["狙擊槍"] = Weapon("狙擊槍", 50, "piercing", 45, "snd_sniper") # 狙擊跟電磁炮以及步槍等槍枝暫時共用音效
...
WEAPON_TYPES["步槍"] = Weapon("步槍", 40, "piercing", 30, "snd_heavy_sniper")
...
class DropItem:
    ...
    def draw(self, surface):
        if self.item_type == "EXP":
            ...
            pygame.draw.rect(surface, HP_COLOR, (self.x-6, self.y-2, 12, 4))
            pygame.draw.rect(surface, HP_COLOR, (self.x-2, self.y-6, 4, 12))
