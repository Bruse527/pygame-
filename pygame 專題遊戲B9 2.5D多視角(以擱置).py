...
def draw_changelog_popup(surface):
    # 直接使用全域設定好的面板與按鈕座標
    draw_ui_panel(surface, changelog_panel, "更新與修復日誌", BLUE)
    draw_hover_button(surface, changelog_close_button, "X", (180, 60, 60), RED, WHITE)
    logs = [
        "修復項目與優化內容:",
        "- 徹底升級：3D 地平線視角與透視網格",
    ...
    for i, line in enumerate(logs):
        surface.blit(small_font.render(line, True, WHITE), (changelog_panel.x + 20, changelog_panel.y + 60 + i * 30))

def draw_pause_upgrade_log(surface):
    draw_upgrade_summary(surface, WIDTH//2 - 120, HEIGHT//2 + 115, max_items=8, title="已獲得的強化")
...
game_settings = {
    "bgm_vol": 5,     # 音量 0-10
    "sfx_vol": 5,     # 音效 0-10
    "enemy_hp": 100,  # 敵人血量百分比 25% - 200%
    "enemy_dmg": 100, # 敵人傷害百分比 25% - 200%
    "enemy_spd": 100  # 敵人速度百分比 25% - 200%
}
previous_state = "MENU" # 紀錄是從主選單還是暫停選單進入設定的
set_items = [
    {"key": "bgm_vol", "label": "音樂音量", "min": 0, "max": 10, "step": 1, "unit": ""},
    {"key": "sfx_vol", "label": "音效音量", "min": 0, "max": 10, "step": 1, "unit": ""},
    {"key": "enemy_hp", "label": "敵人血量", "min": 25, "max": 200, "step": 25, "unit": "%"},
    {"key": "enemy_dmg", "label": "敵人傷害", "min": 25, "max": 200, "step": 25, "unit": "%"},
    {"key": "enemy_spd", "label": "敵人速度", "min": 25, "max": 200, "step": 25, "unit": "%"}
]

def update_volumes():
    pygame.mixer.music.set_volume(game_settings["bgm_vol"] * 0.04) # 5 預設剛好是 0.2
    for s in sounds.values():
        if s: s.set_volume(game_settings["sfx_vol"] * 0.06)  # 5 預設剛好是 0.3
############################################################################################
    ...
    def draw(self, surface, current_wep=None):
        ...
        if draw_player:
            ...
            if img:
                ...
                pygame.draw.rect(surface, YELLOW if self.god_mode else BLUE, pygame.Rect(0, 0, self.size, self.size).move(draw_center[0]-self.size/2, draw_center[1]-self.size/2))
...
class EnemyBullet:
    def __init__(self, x, y, dir_x, dir_y, color=ORANGE, is_homing=False, weapon=None):
        ...
        mult_dmg = game_settings["enemy_dmg"] / 100.0
        self.damage, self.b_type = int(15 * mult_dmg), "normal"
        if weapon:
            self.b_type, self.damage = weapon.bullet_type, int(weapon.damage * 0.8 * mult_dmg)
...
class Enemy:
    def __init__(self, level, is_elite=False, spawn_x=MAP_WIDTH/2, spawn_y=MAP_HEIGHT/2):
        ...
        mult_hp = game_settings["enemy_hp"] / 100.0
        mult_dmg = game_settings["enemy_dmg"] / 100.0
        mult_spd = game_settings["enemy_spd"] / 100.0

        self.speed = ((random.uniform(3.0, 5.5) if is_elite else random.uniform(2.5, 4.5)) + level * 0.05) * (1.2 if game_mode == "CHALLENGE" else 1.0) * mult_spd
        self.max_hp = int(((60 + level * 25) if is_elite else (20 + level * 8)) * difficulty_mult * mult_hp)
        self.hp, self.max_shield = self.max_hp, int(((20 + level * 8) if is_elite else (10 + level * 4)) * difficulty_mult * mult_hp)
        self.shield, self.damage = self.max_shield, int(((35 + level * 3) if is_elite else (15 + level * 1.5)) * difficulty_mult * mult_dmg)
    ...
    def draw(self, surface):
        ...
        if self.combat_type == "kamikaze":
            ...
            if anim_frames:
                ...
                draw_rect = pygame.Rect(0, 0, self.size, self.size); draw_rect.center = draw_center
...
class Boss:
    def __init__(self, boss_type, level, spawn_x=MAP_WIDTH/2, spawn_y=MAP_HEIGHT/2):
        ...
        self.hit_timer = 0
        # 根據遊戲設定調整BOSS的屬性，讓玩家可以透過調整難度來改變遊戲體驗
        mult_hp = game_settings["enemy_hp"] / 100.0
        mult_dmg = game_settings["enemy_dmg"] / 100.0
        mult_spd = game_settings["enemy_spd"] / 100.0
        
        base_hp = {"YELLOW": 3000, "RED": 4000, "PURPLE": 2500, "CYAN": 3200}[self.b_type]
        self.max_hp = int((base_hp + level * 100) * (1.75 if game_mode == "CHALLENGE" else 1.0) * mult_hp)
        self.hp = self.max_hp
        self.damage = int(40 * mult_dmg) 
        
        if self.b_type == "YELLOW": self.color, self.speed, self.state = YELLOW, 3.0 * mult_spd, "EVADE"
        elif self.b_type == "RED": self.color, self.speed, self.state, self.aim_x, self.aim_y = RED, 2.5 * mult_spd, "CHASE", 0, 0
        elif self.b_type == "PURPLE": self.color, self.speed, self.state = PURPLE, 2.0 * mult_spd, "FLEE"
        elif self.b_type == "CYAN": self.color, self.speed, self.state = CYAN, 3.0 * mult_spd, "IDLE"
...
# UI 座標位置自適應
def update_ui_rects():
    #當視窗大小改變時會重新計算所有 UI 面板與按鈕的位置置中
    global start_button, changelog_button, settings_button, exit_button
    global normal_button, challenge_button, difficulty_back_button
    global changelog_panel, changelog_close_button, restart_button
    global pause_btn_continue, pause_btn_settings, pause_btn_menu, pause_btn_giveup, pause_btn_exit
    global btn_settings_back, menu_button, cards, confirm_upgrade_button
    global shop_bg, stash_bg, mod_bg, wep_stash_bg
    global btn_shop_close, btn_stash_close, btn_mod_close, btn_wep_close
    global btn_hp, btn_dmg, btn_spd, btn_stamina, btn_shield, btn_energy
    global list_rect, btn_prim_w, btn_sec_w, rect_prim, rect_sec, upg_btn, reroll_btn
    global s_start_x, s_start_y, p_start_x_s, p_start_y_s, p_start_x_m, p_start_y_m, p_start_x_w, p_start_y_w
    global dim_surface


# 按鈕定義大小與位置
 # 主選單與難度選擇
    start_button = pygame.Rect(WIDTH//2 - 110, HEIGHT//2 + 20, 220, 45)
    changelog_button = pygame.Rect(WIDTH//2 - 110, HEIGHT//2 + 75, 220, 45)
    settings_button = pygame.Rect(WIDTH//2 - 110, HEIGHT//2 + 130, 220, 45) 
    exit_button = pygame.Rect(WIDTH//2 - 110, HEIGHT//2 + 185, 220, 45)
    ...
    changelog_panel = pygame.Rect(WIDTH//2 - 300, HEIGHT//2 - 200, 600, 400)
    changelog_close_button = pygame.Rect(changelog_panel.right - 45, changelog_panel.y + 10, 35, 35)

    # 死亡與暫停
    restart_button = pygame.Rect(WIDTH//2 - 110, HEIGHT//2 + 100, 220, 50)
    menu_button = pygame.Rect(WIDTH//2 - 110, HEIGHT//2 + 170, 220, 50)

    pause_btn_continue = pygame.Rect(WIDTH//2 - 240, HEIGHT//2 - 70, 220, 50)
    pause_btn_settings = pygame.Rect(WIDTH//2 + 20, HEIGHT//2 - 70, 220, 50)
    pause_btn_menu = pygame.Rect(WIDTH//2 - 240, HEIGHT//2 - 10, 220, 50)
    pause_btn_giveup = pygame.Rect(WIDTH//2 + 20, HEIGHT//2 - 10, 220, 50)
    pause_btn_exit = pygame.Rect(WIDTH//2 - 110, HEIGHT//2 + 50, 220, 50)
    btn_settings_back = pygame.Rect(WIDTH//2 - 110, HEIGHT//2 + 180, 220, 50) 
...
while running:
        ...
        if show_inventory and game_state == "PLAYING":
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if show_changelog:
                    ...
                    elif settings_button.collidepoint(event.pos): previous_state = "MENU"; game_state = "SETTINGS"
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if pause_btn_continue.collidepoint(event.pos): game_state = "PLAYING"
                elif pause_btn_settings.collidepoint(event.pos): previous_state = "PAUSED"; game_state = "SETTINGS"
                elif pause_btn_menu.collidepoint(event.pos): full_wipe("NORMAL")
                elif pause_btn_giveup.collidepoint(event.pos): enter_bunker(success=False)
                elif pause_btn_exit.collidepoint(event.pos): running = False

        elif game_state == "SETTINGS":
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if btn_settings_back.collidepoint(event.pos):
                    game_state = previous_state # 點返回回到上一個畫面
                for i, item in enumerate(set_items):
                    btn_minus = pygame.Rect(WIDTH//2 - 40, HEIGHT//2 - 120 + i * 50, 40, 30)
                    btn_plus = pygame.Rect(WIDTH//2 + 100, HEIGHT//2 - 120 + i * 50, 40, 30)
                    if btn_minus.collidepoint(event.pos):
                        game_settings[item["key"]] = max(item["min"], game_settings[item["key"]] - item["step"])
                        play_sound("exp"); update_volumes()
                    if btn_plus.collidepoint(event.pos):
                        game_settings[item["key"]] = min(item["max"], game_settings[item["key"]] + item["step"])
                        play_sound("exp"); update_volumes()
    ...
    if game_state == "BUNKER":
        ...
        for b in bullets:
            ...
            # 子彈爆炸效果的判定放在最前面，確保即使子彈擊中敵人或 Boss 後爆炸效果仍然能觸發
            if b.explode:
                ...
                for e in enemies[:]:
                    if math.hypot(e.x - b.x, e.y - b.y) < 120: 
                        ...
                        if e.hp <= 0: 
                            if random.random() < 0.4: items.append(DropItem(e.x, e.y, "EXP"))
                            if e in enemies: enemies.remove(e)
                # Boss 受到爆炸傷害的判定放在最後，確保 Boss 受到傷害後才觸發爆炸效果，避免 Boss 受到多次爆炸傷害            
            ...
            # Boss 受到子彈傷害的判定放在最後，確保子彈能穿透敵人後再對 Boss 造成傷害
        ...
        if boss_active and player.rect.colliderect(boss.rect) and game_state == "PLAYING": 
            if not player.god_mode and player.invincible_timer <= 0 and not player.is_dashing:
                actual_dmg = max(1, boss.damage - player.damage_reduction)
                if player.shield > 0:
                    if actual_dmg > player.shield: leftover = actual_dmg - player.shield; player.shield = 0; player.hp -= leftover
                    else: player.shield -= actual_dmg
                else: player.hp -= actual_dmg
        ...
        # 統一的玩家死亡判定：無論是否有東西，都會蓋過上一次的遺失物品，確保玩家死亡後只能撿起最新的遺失物品
    ...
    if game_state in ["BUNKER", "SHOP", "GENERAL_STASH", "MOD_STATION", "WEAPON_STASH", "PLAYING", "PAUSED", "LEVEL_UP", "DIED"]:
        ...
        # 純粹的宇宙虛空背景(備用)
        # 繪製天空與地平線/地板 (HORIZON) 的位置會根據視角模式調整，FPS 模式下地平線/地板更低
        # 只有在 3D 模式下才畫出天空與夕陽
        if view_mode != "CLASSIC":
        ...
        # 3D 空間的地圖外夕陽
        # 將夕陽作為一個真實的巨大 3D 發光體，固定放置在世界地圖的東方邊界之外
        sun_world_x = MAP_WIDTH + 1500  # 放在地圖東邊界外 1500 單位
        sun_world_y = MAP_HEIGHT // 2   # Y 軸置中對齊地圖
        sun_world_z = 350               # 懸浮在半空 Z 軸 350 的高度
        ...
        # 如果太陽在相機的前方，就畫出它
        if sun_depth > -1000:
            # 根據玩家距離計算太陽的視覺縮放 (你越靠近地圖邊緣，太陽看起來越大)
            dist_to_sun = math.hypot(sun_world_x - player.x, sun_world_y - player.y)
            sun_radius = max(60, int(150000 / max(100, dist_to_sun))) 
            
            # 畫出 3D 太陽本體與光暈
            pygame.draw.circle(screen, (255, 80, 80), (int(sun_sx), int(sun_sy)), sun_radius)
            pygame.draw.circle(screen, (255, 80, 80, 80), (int(sun_sx), int(sun_sy)), int(sun_radius * 1.4), max(2, sun_radius//8))
        
        # 繪製浮空地圖邊界與地板 (限縮於 MAP_WIDTH / MAP_HEIGHT)
        ...
        if floor_img:
            # 配合相機旋轉與視角壓扁，將整張地圖圖片貼上  我想做出類似mode 7 的效果
            ...
            pygame.draw.polygon(screen, (10, 12, 18), proj_corners) # 備用幾何地板
            
        pygame.draw.polygon(screen, (0, 200, 255), proj_corners, 2) # 發光世界邊緣
        
        # 網格限定在世界邊界內
        grid_size = 150
        for x in range(0, MAP_WIDTH + 1, grid_size):
            p1 = project_3d(x, 0)
            p2 = project_3d(x, MAP_HEIGHT)
            if p1[2] > -5000 or p2[2] > -5000:
                pygame.draw.line(screen, (30, 45, 60), p1[:2], p2[:2], 2)
                
        for y in range(0, MAP_HEIGHT + 1, grid_size):
            p1 = project_3d(0, y)
            p2 = project_3d(MAP_WIDTH, y)
            if p1[2] > -5000 or p2[2] > -5000:
                pygame.draw.line(screen, (30, 45, 60), p1[:2], p2[:2], 2)
        ...
        if game_state in ["BUNKER", "SHOP", "GENERAL_STASH", "MOD_STATION", "WEAPON_STASH"]:
            # 畫出安全區的高科技雷射圍牆
            bx, by, bw, bh = MAP_WIDTH//2 - 500, MAP_HEIGHT//2 - 400, 1000, 800
            ...
            for i in range(4):
                ...
                s1_top = project_3d(p1[0], p1[1], 200) # 牆高 
                s2_top = project_3d(p2[0], p2[1], 200)
                ...
                if s1_bot[2] > -5000 or s2_bot[2] > -5000:
                    ...
                    if wall_img:
                        ...
                        # 畫出不透明的深灰色金屬牆面阻擋後方的視線產生封閉感
                        pygame.draw.polygon(screen, (35, 40, 48), wall_polygon)
                        
                        # 畫出牆壁頂部粗壯的承重鋼梁
                        pygame.draw.line(screen, (55, 60, 70), s1_top[:2], s2_top[:2], 8)
                        
                        # 畫出牆角底部的深色陰影縫隙
                        pygame.draw.line(screen, (15, 18, 22), s1_bot[:2], s2_bot[:2], 10)
                        
                        # 4底部加上工廠常見的黃黑相間警告條紋 (利用線段切分模擬)
                        dx = s2_bot[0] - s1_bot[0]
                        dy = s2_bot[1] - s1_bot[1]
                        dist = math.hypot(dx, dy)
                        if dist > 0:
                            for step in range(0, int(dist), 50): # 每 50 像素畫一段黃線
                                px1 = s1_bot[0] + (dx / dist) * step
                                py1 = s1_bot[1] + (dy / dist) * step
                                px2 = s1_bot[0] + (dx / dist) * min(step + 25, dist)
                                py2 = s1_bot[1] + (dy / dist) * min(step + 25, dist)
                                pygame.draw.line(screen, (200, 180, 0), (px1, py1), (px2, py2), 4)

                        # 在牆面中間加上黯淡的 那是橘紅色工業照明燈管
                        mid_h = 100 # 燈管高度設在 Z 軸 100
                        sm1 = project_3d(p1[0], p1[1], mid_h)
                        sm2 = project_3d(p2[0], p2[1], mid_h)
                        pygame.draw.line(screen, (180, 50, 20), sm1[:2], sm2[:2], 3) # 燈管實體
                        
                        # 利用半透明畫出燈光暈染效果
                        glow_surf = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
                        pygame.draw.line(glow_surf, (180, 50, 20, 80), sm1[:2], sm2[:2], 12)
                        screen.blit(glow_surf, (0, 0))

                        # 在牆壁兩側畫出極粗的黑色承重支柱，將每面牆連接起來
                        pygame.draw.line(screen, (20, 22, 28), s1_bot[:2], s1_top[:2], 16)
                        pygame.draw.line(screen, (20, 22, 28), s2_bot[:2], s2_top[:2], 16)
    ...
    if game_state == "MENU":
        ...
        draw_hover_button(screen, settings_button, "遊戲設定", (150, 100, 50), ORANGE)
    ...
    # 暫停畫面的四個選項：繼續遊戲、回到選單、放棄重製（回地堡）和退出遊戲。這些選項提供了玩家在暫停時的不同選擇，讓他們可以根據當前情況做出最適合的決定。
        ...
        draw_hover_button(screen, pause_btn_continue, "繼續遊戲", (50, 100, 150), BLUE)
        draw_hover_button(screen, pause_btn_settings, "遊戲設定", (150, 100, 50), ORANGE)
        draw_hover_button(screen, pause_btn_menu, "回到主選單", (50, 100, 150), BLUE)
        draw_hover_button(screen, pause_btn_giveup, "放棄重製(回地堡)", (50, 150, 50), GREEN)
        draw_hover_button(screen, pause_btn_exit, "退出遊戲", (150, 50, 50), RED)
        draw_pause_upgrade_log(screen)
    
#遊戲設定畫面繪製 (SETTINGS)
    elif game_state == "SETTINGS":
        screen.blit(dim_surface, (0, 0))
        panel_rect = pygame.Rect(WIDTH//2 - 250, HEIGHT//2 - 180, 500, 430)
        
        draw_ui_panel(screen, panel_rect, "遊戲設定", YELLOW)
        
        for i, item in enumerate(set_items):
            y_pos = HEIGHT//2 - 120 + i * 50
            # 項目標題
            screen.blit(font.render(item["label"], True, WHITE), (WIDTH//2 - 180, y_pos + 2))
            
            # 減號與加號按鈕
            btn_minus = pygame.Rect(WIDTH//2 - 40, y_pos, 40, 30)
            btn_plus = pygame.Rect(WIDTH//2 + 100, y_pos, 40, 30)
            draw_hover_button(screen, btn_minus, "-", GRAY, WHITE, BLACK)
            draw_hover_button(screen, btn_plus, "+", GRAY, WHITE, BLACK)
            
            # 中間顯示當前數值 (置中對齊在 - 和 + 之間)
            val_text = f"{game_settings[item['key']]}{item['unit']}"
            val_surf = font.render(val_text, True, YELLOW)
            screen.blit(val_surf, (WIDTH//2 + 50 - val_surf.get_width()//2, y_pos + 2))
            
        draw_hover_button(screen, btn_settings_back, "返回", (50, 100, 150), BLUE)
