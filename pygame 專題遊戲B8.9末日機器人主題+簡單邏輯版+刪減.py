...
class ChargerBoss:
    ...
    def update(self, player_x, player_y, bullets, enemies, enemy_bullets):
        ...
        # 狀態三：高速衝刺
            ...
            # 衝刺速度設 20.0 (原本設太高結果閃不過，先下修)
            self.x += self.dash_dir_x * 20.0
            self.y += self.dash_dir_y * 20.0
...
guide_text_lines = [
    "[遊戲概念]",
    "我們製作的這一款以遊戲是以末日肉鴿(Roguelite)2D射擊類生存挑戰元氣騎士為概念的遊戲。背景是玩家在一個被機器人入侵的世界末日中，躲在地堡裡強化自己對抗外面源源不絕的敵人。",
    ...
    "",
    "[簡要開發日誌]",
    "從開學期初開始製作總共花了超過105小時以上製作，原本還有更多功能蛋餅有點化的太大了，所以後來刪減了很多內容才完成。",
    ...
    "製作最久的功能是戰鬥系統與BOSS，從最初的簡單射擊演變為現在包含多種子彈機制、三大特色BOSS的機制，還有所有的UI介面也花了非常多時間因為找不到滿意的圖片設計，只好直接寫在遊戲中在逐一微調。",
    "因為沒有足夠時間繪製圖片，圖片素材使用的是免費素材，使用 itch.io 網站上的免費素材，音效則是使用 pixabay 上的免費音效。",
    "參考資料大多是pygame遊戲製作相關文章以及，YT遊戲製作影片與B站還有以前製作過的遊戲",
    "",
    "(架構基礎)",
    ...
    "音效框架:建立 64 音效通道，讓獨立短促音效、Boss 專屬背景音樂以及槍聲表現可以正常。",
    "",
    "(維護與優化)",
...
while running == True:
    ...
    # 遊戲主畫面
        ...
        for c in chests: c.draw(screen) # 箱子要先於敵人繪製，這樣玩家才不會被箱子擋住看不到敵人
        for it in items: it.draw(screen) # 道具要先於敵人繪製，這樣玩家才不會被道具擋住看不到敵人
        for p in particles: p.draw(screen) # 爆炸粒子要先於敵人繪製，這樣玩家才不會被粒子擋住看不到敵人
        for b in bullets: b.draw(screen) # 玩家子彈要先於敵人繪製，這樣玩家才不會被自己子彈擋住看不到敵人
        for eb in enemy_bullets: eb.draw(screen) # 敵人子彈要先於敵人繪製，這樣玩家才不會被敵人子彈擋住看不到敵人
        for e in enemies: e.draw(screen) # 怪物要先於玩家繪製，這樣玩家才不會被怪物擋住看不到自己
        for t in trails: t.draw(screen) # 軌跡要先於玩家繪製，這樣玩家才不會被軌跡擋住看不到自己
        for dt in damage_texts: dt.draw(screen) # 傷害數字要先於玩家繪製，這樣玩家才不會被傷害數字擋住看不到自己
        # BOSS 
        if isBossActive:
            boss.draw(screen)
