...
loadAnimation("enemy_normal", "enemy_normal", (60, 60))  # 普通敵人動畫
loadAnimation("enemy_elite", "enemy_elite", (80, 80))  # 菁英敵人動畫
loadAnimation("enemy_kamikaze", "enemy_kamikaze", (60, 60)) # 自爆怪動畫
loadAnimation("dummy", "dummy", (55, 80))  # 訓練假人動畫

loadAnimation("boss_yellow", "boss_yellow", (155, 155))    # 第一隻 Boss
...
def rebuildChangelogCache(w, h):
    pass 

# 繪製指南與介紹的彈出面板
def drawGuidePopup(surface):
    global changelogMaxScroll
    # 把面板放大，容納更多說明文字
    ...
    # 建立一個隱藏的長條畫布用來存放所有文字
    content_surf = pygame.Surface((view_rect.width, total_height), pygame.SRCALPHA)
    ...
    for i, line in enumerate(guide_text_lines):
        ...
        txt_surf = small_font.render(line, True, color)
        content_surf.blit(txt_surf, (0, i * line_height))

    # 只會把目前滾動到的區域貼到螢幕上
    surface.blit(content_surf, view_rect.topleft, pygame.Rect(0, changelogScroll, view_rect.width, view_rect.height))
...
# 假人靶子類別，主要用於測試和展示玩家的傷害輸出，會在被攻擊時記錄傷害並顯示DPS，震動效果來模擬被擊中的感覺
class DummyTarget:
    ...
    def draw(self, surface):
        ...
        dx = int(self.x - camX) + rx
        dy = int(self.y - camY) + ry
        ...
        if len(self.hit_log) > 0: dps = int(total_dmg / 3.0)
        else: dps = 0
        
        if dps > 0: tc = CYAN
        else: tc = GRAY
        
        dps_txt = small_font.render("DPS: " + str(dps), True, tc)
...
class EnemyBullet:
    ...
    # 根據子彈的類型和屬性繪製不同的圖案例如圓形、菱形、斜線或核心，還有根據是否暴擊使用不同的顏色和效果    
    def draw(self, surface): 
        draw_center = (int(self.rect.centerx - camX), int(self.rect.centery - camY))
        ...
        if "enemy_bullet" in images and images["enemy_bullet"] != None:
            ...
            if hasattr(self, 'style'):
                if self.style == "diamond":
                    ...
                    cR = self.radius // 2
                    if cR < 3: cR = 3
                    pygame.draw.circle(surface, getattr(self, 'core_color', WHITE), draw_center, cR)
            else:
                cR = self.radius // 2
                if cR < 3: cR = 3
                pygame.draw.circle(surface, getattr(self, 'core_color', WHITE), draw_center, cR)
...
class Enemy:
    ...
    def draw(self, surface):
        ...
        if self.combat_type == "kamikaze":
            # 優先嘗試畫出圖片
            if "enemy_kamikaze" in animations and animations["enemy_kamikaze"] != None:
                anim_frames = animations["enemy_kamikaze"]
                img = anim_frames[int(pygame.time.get_ticks() / 100) % len(anim_frames)]
                ...
                if self.hit_timer > 0:
                    ...
                    pygame.draw.circle(flash, (255, 255, 255, 150), (self.size//2, self.size//2), self.size//2)
                    surface.blit(flash, flash.get_rect(center=draw_center), special_flags=pygame.BLEND_RGBA_ADD)
                # 冰凍效果
                elif self.frost_timer > 0:
                    flash = pygame.Surface((self.size, self.size), pygame.SRCALPHA)
                    pygame.draw.circle(flash, (100, 200, 255, 100), (self.size//2, self.size//2), self.size//2)
                    surface.blit(flash, flash.get_rect(center=draw_center), special_flags=pygame.BLEND_RGBA_MULT)
            else:
                # 如果沒有圖片就畫原本的像素幾何炸彈
            ...
            # 顯示敵人圖片
            if self.is_elite: anim_key = "enemy_elite"
            ...
            if anim_key in animations and animations[anim_key] != None:
                ...
                elif self.is_elite: color = (150, 0, 150)
                else: color = RED
        ...
        # 繪製護盾條和血條 護盾條在血條上方如果有護盾則會顯示藍色的部分
        if self.max_shield > 0 and self.shield > 0:
            sw = self.size * (self.shield/self.max_shield)
            if sw < 0: sw = 0
            pygame.draw.rect(surface, GRAY, (draw_rect.x, draw_rect.y - 14, self.size, 4))
            pygame.draw.rect(surface, (0, 150, 255), (draw_rect.x, draw_rect.y - 14, sw, 4))
        # 血條永遠顯示，但如果血量為滿則不會有綠色部分    
        if self.hp < self.max_hp: 
            hw = self.size * (self.hp/self.max_hp)
            if hw < 0: hw = 0
            pygame.draw.rect(surface, GRAY, (draw_rect.x, draw_rect.y - 8, self.size, 4))
            pygame.draw.rect(surface, GREEN, (draw_rect.x, draw_rect.y - 8, hw, 4))
...
class CoreBoss:
    ...
    def draw(self, surface):
        ...
        if "boss_yellow" in animations and animations["boss_yellow"] != None:
            ...
            # 如果沒有圖片，畫一個黃色的大圓圈代替
            pygame.draw.circle(surface, YELLOW, (cx, cy), self.size // 2)
            pygame.draw.rect(surface, RED, (cx - 10, cy - 10, 20, 20))
...
# 像機械公牛衝刺行敵人會 鎖定玩家 -> 停止 -> 高速衝撞 -> 衝撞時背後發射子彈
class ChargerBoss:
    def __init__(self, spawn_level=5, player_x=0, player_y=0):
        self.x = player_x + 600
        self.y = player_y
    ...
    def update(self, player_x, player_y, bullets, enemies, enemy_bullets):
        ...
        # 狀態三：高速衝刺
            ...
            # 衝刺速度設 15.0 (原本設太高結果閃不過，先下修)
            self.x += self.dash_dir_x * 15.0
            self.y += self.dash_dir_y * 15.0
    ...
    def draw(self, surface):
        ...
        if "boss_charger" in animations and animations["boss_charger"] != None:
            ...
            pygame.draw.rect(surface, ORANGE, (cx - 40, cy - 40, 80, 80))
...
# 機制是以系統故障的機器人做為參考，會發射追蹤飛彈，並隨機閃現瞬移到玩家附近爆發攻擊
class BerserkerBoss:
    def __init__(self, spawn_level=5, player_x=0, player_y=0):
        self.x = player_x
        self.y = player_y - 400
    ...
    def update(self, player_x, player_y, bullets, enemies, enemy_bullets):
        ...
        if self.state == "WALK":
            ...
            # 每 40 幀發射兩顆追蹤飛彈
            if self.timer % 40 == 0:
                enemy_bullets.append(EnemyBullet(self.x, self.y, 1, 0, color=PURPLE, is_homing=True))
                enemy_bullets.append(EnemyBullet(self.x, self.y, -1, 0, color=PURPLE, is_homing=True))
                playSound("shoot_laser")
                
            # 走 180 個 frame  3秒 後進行瞬移
            if self.timer > 180:
                self.state = "TELEPORT"
                self.timer = 0
        ...
        # 狀態二：故障瞬移與全方位爆發 
            ...
            if self.timer == 1:
                # 第 1 幀瞬間改變座標 (傳送到玩家附近隨機位置)
                self.x = player_x + random.randint(-300, 300)
                self.y = player_y + random.randint(-300, 300)
                # 確保不跑出地圖
                if self.x < 100: self.x = 100
                if self.x > mapWidth - 100: self.x = mapWidth - 100
                if self.y < 100: self.y = 100
                if self.y > mapHeight - 100: self.y = mapHeight - 100
                # 發出閃現的聲音
                playSound("dash")
                
            # 落地後停頓半秒，然後向周圍炸出 12 顆子彈
            if self.timer == 30:
                for i in range(12):
                    angle = i * (360 / 12)
                    rad = math.radians(angle)
                    dir_x = math.cos(rad)
                    dir_y = math.sin(rad)
                    enemy_bullets.append(EnemyBullet(self.x, self.y, dir_x, dir_y, color=RED))
                playSound("shoot_cannon")
    ...
    def can_take_damage(self):
        # 死亡後無敵
        if self.state == "DEFEAT": return False
        if self.state == "TELEPORT" and self.timer < 15: return False
        return True

    def get_intro_title(self): return self.name + " 偵測到目標！"
    def get_state_message(self):
        if self.state == "TELEPORT": return "空間跳躍中！", PURPLE
        return "狂亂終結者 - 追蹤飛彈", RED
...
guide_text_lines = [
    "[遊戲概念]",
    "這是一款以末日肉鴿(Roguelite)像素2D射擊類生存為概念的遊戲。背景是玩家在一個被機器人入侵的世界末日中，躲在地堡裡強化自己對抗外面源源不絕的敵人。",
    "透過擊敗敵人以及尋找寶箱獲取獎勵來不斷強化自身。每次挑戰都會遭遇隨機的敵人與3種BOSS，",
    ...
    "",
    "[戰鬥與機制]",
    "超時事件: 若存活時間超過限制，將觸發BOSS軍隊事件，大量精英包圍。",
    ...
    "共有12種特色武器，從基礎手槍、散彈，到雷射、冰霜槍與大砲。",
    "",
    "[敵人與 BOSS]",
    "敵人分近戰、遠程與自爆三種。難度會隨玩家等級動態調整。",
    "三大Boss會在玩家每次等級提升5級時出現: ",
    "1. 核心機器人: 基礎彈幕射擊型。",
    "2. 衝刺突擊者: 高機動性瘋狗模式，具備硬直空窗期。",
    ...
    "[難度差異]",
    "{{為了方便測試升級以及槍枝數值有提高}}",
    "普通模式:基礎倍率 1.0x，無彈藥限制，適合新手農怪。",
    "挑戰模式:敵人強度 1.75x 且有速度加成，啟動換彈懲罰機制，解鎖專屬卡牌。",
    "",
    "[簡要開發日誌]",
    "從期初開始製作總共花了超過100小時製作(超過100小時候就沒有統計了)，原本還有更多功能蛋餅有點化的太大了，所以後來刪減了很多內容才完成。",
    "主要歷經A1、A2、A3、A4、A5、B1、B2、B3、B4、B5、B6、B7、B8、B8.5、B9(以擱置的3D功能版)、接續B8.5的B8.6、B8.7、B8.8、B8.9、C1、C2多個版本迭代，從單視窗擴充為 4200x2600 開放世界。",
    "製作最久的功能是戰鬥系統與BOSS，從最初的簡單射擊演變為現在包含多種子彈機制、三大特色BOSS的機制，還有所有的UI介面也花了很多時間。",
    "(架構基礎)",
    "遊戲基礎設定:從最初的 800x600 視窗進化至 1920x1080 解析度，並支援自適應全螢幕切換。",
    "(戰鬥系統)",
    "移動與瞄準: WASD 八方向移動與滑鼠向量射擊（利用三角函數計算軌跡）。",
    "子彈機制:加入追蹤邏輯（可隨升級強化強度）、武器詞綴屬性、物理擊退效果與彈性傷害數值顯示。",
    "感官回饋:加入卡肉感(Hit Stop)、受擊閃白(Hit Flash)、以及子彈擊中時的粒子特效。",
    "(進階 Roguelite 與 RPG 玩法)",
    "Roguelite 成長系統:導入 25 種強化卡牌(如子彈分裂、無人機等)，建立升級時的點擊選擇介面。",
    "地圖與探索:擴展為 4200x2600 的開放世界，包含動態相機跟隨、小地圖(實時顯示玩家、撤離點、Boss 與遺失物)以及 Boss 方向警示系統。",
    "死亡與回收機制:採用類魂系死亡機制，玩家死亡後物品會掉落在地，需回到原處拾回；新增撤離系統，成功撤離可保留物資。",
    "RPG 元素:新增對話系統與隨機任務系統，玩家在地堡可接取任務並獲取獎勵。",
    "(資源與 UI 管理) ",
    "背包與商店系統:24 格實體背包，支援滑鼠拖曳整理、物品變賣、以及黑市商店與武器改造台。",
    "物資系統:新增護盾(具備脫戰自動恢復)、血包、各式武器(含槍枝與特效)，並將物品拾取調整為自動吸附模式。",
    "(Boss 戰與音效):",
    "Boss 機制：原本設計了四款BOSS，最終實裝了三個比較有特色的BOSS 根據玩家等級動態調整難度。",
    "音效框架:建立 64 音效通道，支援獨立短促音效、Boss 專屬背景音樂以及動態起伏的槍聲表現。",
    "(維護與優化)",
    "效能調校:修復了因介面重繪導致的掉幀問題(主選單光暈重繪)、碰撞運算優化，以及視野外的實體自動清除。",
    "Bug 修復:整合了遺失物回收的安全鎖機制(防止裝備消失)、修復碰撞穿模、UI 對齊以及動畫顯示異常等問題",
    "遊戲視窗支援無損自適應全螢幕，並加入防爆音與動態槍聲機制。",
]
...
while running == True:
    ...
    if gameState == "BUNKER":
        ...
        if mouse_btns[0] and shootCooldown <= 0 and player.is_dashing == False:
            ...
            for i in range(t_bullets):
                ...
                for j in range(1 + player.extra_same_path_bullets):
                    off = s_dir * (j * 15)
                    tx = player.x + s_dir.x * 100 + off.x
                    ty = player.y + s_dir.y * 100 + off.y
            ...
            if can_fire == True:
                ...
                for i in range(t_bullets):
                    ...
                    for j in range(1 + player.extra_same_path_bullets):
                        ...
                        tx = player.x + s_dir.x * 100 + spawn_offset.x
                        ty = player.y + s_dir.y * 100 + spawn_offset.y
        ...
        if player.drone_level > 0:
            ...
            if player.drone_shoot_cd <= 0 and len(enemies) > 0:
                closest = None
                minD = 99999
                for e in enemies:
                ...
                if closest != None and minD < 400:
                    drone_x = player.x + math.cos(player.drone_angle) * 55
                    drone_y = player.y + math.sin(player.drone_angle) * 55
                    temp_wep = generateWeapon("手槍", "白")
                    temp_wep.bullet_type = "normal"
                    temp_wep.damage = 10 + player.drone_level * 8
                    bullets.append(Bullet(drone_x, drone_y, closest.x, closest.y, temp_wep))
                    player.drone_shoot_cd = 60 - player.drone_level * 10
                    if player.drone_shoot_cd < 10: player.drone_shoot_cd = 10
        ...
        for b in bullets:
            ...
            temp_enemies = []
            for e in enemies:
                if e.hp > 0: temp_enemies.append(e)
            enemies = temp_enemies
                    
            if isBossActive and b.rect.colliderect(boss.rect):
                hit_something = True
                if boss.can_take_damage() == False:
                    ...
                    if boss.hp <= 0 and hasattr(boss, "survive_lethal_damage") and boss.survive_lethal_damage():
                        for i in range(28): particles.append(Particle(boss.x, boss.y, RED))
        ...
        if isBossActive and boss != None:
            ...
            if resolveChestCollision(boss, chests):
                if boss.state == "CHARGE" or boss.state == "DASH" or boss.state == "SLAM" or boss.state == "RAGE_DASH":
                    ...
                    if b_name == "旋轉彈幕核心": boss.state = "EVADE"
                    elif b_name == "衝刺突擊者": boss.state = "RECOVER"
                    else: boss.state = "HUNT"
                    boss.state_timer = 0
                    screenShake = 15
                    playSound("shoot_cannon") 
    ...
    if gameState == "BUNKER" or gameState == "SHOP" or gameState == "GENERAL_STASH" or gameState == "MOD_STATION" or gameState == "WEAPON_STASH" or (gameState == "DIALOGUE" and dialogueSys.previous_state == "BUNKER"):
            ...
            for c in chests:
                if c.state == "CLOSED":
                    if math.hypot(player.x - c.x, player.y - c.y) < 70:
                        ...
                        bg_r = pygame.Rect(c.x - camX - 40, c.y - camY - 50, font.size(t)[0]+20, 25)
        ...
        if extractionPt != None:
            ...
            screen.blit(large_font.render("撤離倒數: " + ms + ":" + ss, True, tc), (WIDTH//2 - 120, 20))
    ...
    if gameState == "MENU":
        ...
        for card in cards:
            ...
            # 顏色計算 這是為了了讓升級選項的顏色在被選取或滑鼠懸停時能夠變亮一些
            r = base_color[0] + 35
            if r > 255: r = 255
            g = base_color[1] + 35
            if g > 255: g = 255
