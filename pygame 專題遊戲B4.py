...
class Bullet:
    def __init__(self, x, y, target_x, target_y, weapon):
        ...
        if self.b_type in["piercing", "laser", "cannon", "flamethrower"]:
        ...
        if self.b_type == "piercing": 
            self.color = PURPLE; self.speed = 28; self.radius = 7
        elif self.b_type == "flamethrower": 
            self.color = ORANGE; self.speed = 12; self.radius = 12; self.lifespan = 25
        elif self.b_type == "laser": 
            self.color = CYAN; self.speed = 45; self.radius = 4
        elif self.b_type == "cannon": 
            self.color = WHITE; self.speed = 12; self.radius = 20
        elif self.b_type == "frost": 
            self.color = (100, 200, 255); self.speed = 16; self.radius = 8
        elif self.b_type == "flame_grenade": 
            self.color = RED; self.speed = 10; self.radius = 10
        elif self.b_type == "plasma": 
            self.color = GREEN; self.speed = 15; self.radius = 10
    ...
    def update(self):
        ...
        if self.b_type == "plasma":
            if self.x <= 0 or self.x >= WIDTH: self.dir_x *= -1
            if self.y <= 0 or self.y >= HEIGHT: self.dir_y *= -1
...
class Enemy:
    ...
    def update(self, target_x, target_y, all_enemies):
        ...
        # 因該可以解決怪物疊羅漢，簡易排斥力測試中
        for other in all_enemies:
            if other is not self:
                # 計算與其他怪物的距離平方 (省效能寫法)
                dist_sq = (self.x - other.x)**2 + (self.y - other.y)**2
                min_dist = self.size # 如果距離小於自身體積，產生互斥力
    ...
    def update(self, player_x, player_y, bullets, enemies, enemy_bullets):
        ...
        if self.b_type == "YELLOW":
                ...
                for i in range(len(bullets)):
                    ...
                    if b_dist < 150:
                        ...
                        if flee_dist > 0:
                            ...
                            self.x += flee_dir_x * (current_speed * 1.8)
                            self.y += flee_dir_y * (current_speed * 1.8)
                        dodged = True
                        break 
                        
                if dodged == False:
                    self.x += tangent_x * current_speed
                    self.y += tangent_y * current_speed
                    p_dist = math.sqrt((self.x - player_x)**2 + (self.y - player_y)**2)
                    if p_dist > 250: 
                        self.x += dir_x * current_speed
                        self.y += dir_y * current_speed
                    elif p_dist < 150: 
                        self.x -= dir_x * current_speed
                        self.y -= dir_y * current_speed
            ...
            if self.state == "CHASE":
                ...
                self.x += self.dash_dir_x * (current_speed * 6) 
                self.y += self.dash_dir_y * (current_speed * 6)
...
class DamageText:
    ...
    def draw(self, surface):
        if self.timer > 0:
            ...
            txt_surf.set_alpha(self.alpha) # 設定透明度
    ...
    def draw(self, surface):
        ...
        if img:
            ...
            if self.item_type == "EXP":
                pts =[(self.x, float_y-6), (self.x+6, float_y), (self.x, float_y+6), (self.x-6, float_y)]
...
upgrade_options =[
    ...
    {"title": "能量擴容", "desc":["最大能量 +50", "施放更多技能"]}
]
...
while running:
    ...
    if game_state == "PLAYING":
        ...
        for i in range(len(bullets) - 1, -1, -1):
            ...
            if b.lifespan <= 0 or not screen.get_rect().colliderect(b.rect):
                bullets.pop(i)
        ...
        for i in range(len(bullets) - 1, -1, -1):
            ...
            for j in range(len(enemies) - 1, -1, -1):
                ...
                if b.rect.colliderect(e.rect) == True:
                    ...
                    e.hp -= b.damage
                    # 產生傷害跳字，傷害超過40顯示黃色，否則白色
                    txt_color = YELLOW if b.damage >= 40 else WHITE
                    damage_texts.append(DamageText(e.x, e.y - 20, b.damage, txt_color))
                    hit_something = True
                    for _ in range(5): 
        ...
        for i in range(len(items) - 1, -1, -1):
            ...
            if player.rect.colliderect(g.rect) == True:
                items.remove(g)
                player.exp += 15
                play_sound("exp") 
    ...
    if boss_active == True:
        ...
        if boss.b_type == "YELLOW":
            boss_name = "幾何守衛"
        elif boss.b_type == "RED":
            boss_name = "鮮血狂戰士"
        elif boss.b_type == "PURPLE":
            boss_name = "虛空召喚師"
            
        boss_str = "警告：偵測到極度危險異常實體 - 【" + boss_name + "】"
