...
sounds["shoot"] = sounds.get("shoot_normal")
...
def draw_hover_button(surface, rect, text, base_color, hover_color, text_color=WHITE):
    def wrap_text(content, font_obj, max_width):
        if font_obj.size(content)[0] <= max_width:
            return [content]
        words = content.split(" ")
        lines = []
        current = ""
    ...
    max_text_width = max(10, rect.width - 16)
    lines = wrap_text(text, font, max_text_width)
    text_font = font
    if len(lines) > 2 or any(text_font.size(line)[0] > max_text_width for line in lines):
        lines = wrap_text(text, small_font, max_text_width)
        text_font = small_font
    if len(lines) > 2 or any(text_font.size(line)[0] > max_text_width for line in lines):
        lines = wrap_text(text, tiny_font, max_text_width)
        text_font = tiny_font

    total_height = sum(text_font.size(line)[1] for line in lines) + (len(lines) - 1) * 2
    y = rect.centery - total_height // 2
    for line in lines:
        t_surf = text_font.render(line, True, text_color)
        surface.blit(t_surf, (rect.centerx - t_surf.get_width() // 2, y))
...
class Task:
    ...
    def add_progress(self, amount):
        ...
        self.current_progress = min(self.objective_value, self.current_progress + amount)
        self.is_completed = self.current_progress >= self.objective_value
        return self.is_completed and not old_completed

    def apply_reward(self, player_obj):
        if self.reward_claimed or not self.is_completed:
            return False
        if self.reward_type == "scrap":
            persistent_stats["scrap"] += self.reward_amount
        elif self.reward_type == "exp":
            player_obj.exp += self.reward_amount * player_obj.exp_multiplier
        elif self.reward_type == "max_hp":
            persistent_stats["max_hp"] += self.reward_amount
            ...
            persistent_stats["dmg_bonus"] += self.reward_amount
            player_obj.bullet_damage_bonus += self.reward_amount
        elif self.reward_type == "max_stamina":
            persistent_stats["max_stamina"] += self.reward_amount
            player_obj.max_stamina += self.reward_amount
            player_obj.stamina = min(player_obj.max_stamina, player_obj.stamina + self.reward_amount)
        self.reward_claimed = True
        return True

class TaskSystem:
    TASK_DATA = [
        {"name": "廢料獵人", "desc": "收集 30 個廢料", "type": "collect_scrap", "value": 30, "reward_type": "scrap", "reward": 100},
        {"name": "廢料販子", "desc": "收集 50 個廢料", "type": "collect_scrap", "value": 50, "reward_type": "scrap", "reward": 150},
        {"name": "金鑰匙收藏家", "desc": "收集 5 個金鑰匙", "type": "collect_key", "value": 5, "reward_type": "scrap", "reward": 80},
        {"name": "初級獵人", "desc": "消滅 20 個敵人", "type": "kill", "value": 20, "reward_type": "exp", "reward": 50},
        {"name": "中級獵人", "desc": "消滅 50 個敵人", "type": "kill", "value": 50, "reward_type": "scrap", "reward": 120},
        {"name": "精英獵人", "desc": "消滅 15 個精英敵人", "type": "kill_elite", "value": 15, "reward_type": "dmg_bonus", "reward": 3},
        {"name": "武裝分子", "desc": "造成 5000 點傷害", "type": "damage", "value": 5000, "reward_type": "scrap", "reward": 100},
        {"name": "死神來臨", "desc": "造成 10000 點傷害", "type": "damage", "value": 10000, "reward_type": "dmg_bonus", "reward": 2},
        {"name": "強身計畫", "desc": "升級最大血量 1 次", "type": "upgrade_hp", "value": 1, "reward_type": "max_hp", "reward": 20},
        {"name": "體能訓練", "desc": "升級最大體力 1 次", "type": "upgrade_stamina", "value": 1, "reward_type": "max_stamina", "reward": 20},
        {"name": "長期作戰", "desc": "在突襲中存活 5 分鐘", "type": "survive", "value": 300, "reward_type": "scrap", "reward": 90},
    ]

    def __init__(self):
        self.tasks = [
            Task(i, data["name"], data["desc"], data["type"], data["value"], data["reward_type"], data["reward"])
    ...
    def update_progress(self, objective_type, amount=1):
        ...
        for task in self.tasks:
            if task.objective_type == objective_type:
                completed_any = task.add_progress(amount) or completed_any
        return completed_any

    def claim_completed_rewards(self, player_obj):
        claimed_any = False
        for task in self.tasks:
            if task.is_completed and not task.reward_claimed:
                claimed_any = task.apply_reward(player_obj) or claimed_any
        return claimed_any

    def visible_tasks(self, max_items=5):
        active = [task for task in self.tasks if not task.reward_claimed]
        if active:
            return active[:max_items]
        return self.tasks[-max_items:]

    def completed_count(self):
        return sum(1 for task in self.tasks if task.reward_claimed)

def reward_text_for_task(task):
    if task.reward_type == "scrap":
        return f"{task.reward_amount} 廢料"
    if task.reward_type == "exp":
        return f"{task.reward_amount} 經驗"
    if task.reward_type == "dmg_bonus":
        return f"+{task.reward_amount} 傷害"
    if task.reward_type == "max_hp":
        return f"+{task.reward_amount} 血量"
    if task.reward_type == "max_stamina":
        return f"+{task.reward_amount} 體力"
    return str(task.reward_amount)

def draw_task_panel(surface, task_system, x, y, max_items=5):
    panel_width, panel_height = 330, 230
    panel = pygame.Surface((panel_width, panel_height), pygame.SRCALPHA)
    pygame.draw.rect(panel, (18, 20, 26, 230), panel.get_rect(), border_radius=8)
    pygame.draw.rect(panel, (100, 200, 100), panel.get_rect(), 2, border_radius=8)
    surface.blit(panel, (x, y))

    title = f"任務列表 {task_system.completed_count()}/{len(task_system.tasks)}"
    surface.blit(font.render(title, True, (100, 220, 120)), (x + 14, y + 10))

    row_y = y + 45
    for task in task_system.visible_tasks(max_items):
        progress_percent = min(1.0, task.current_progress / task.objective_value) if task.objective_value else 1
        name_color = GREEN if task.reward_claimed else YELLOW if task.is_completed else WHITE
        status = "已領取" if task.reward_claimed else "完成" if task.is_completed else f"{int(task.current_progress)}/{task.objective_value}"
        name_surf = small_font.render(task.name, True, name_color)
        status_surf = tiny_font.render(status, True, name_color)
        surface.blit(name_surf, (x + 14, row_y))
        surface.blit(status_surf, (x + panel_width - status_surf.get_width() - 14, row_y + 2))

        desc = tiny_font.render(task.description, True, GRAY)
        surface.blit(desc, (x + 14, row_y + 20))
        reward = tiny_font.render(f"獎勵: {reward_text_for_task(task)}", True, YELLOW)
        surface.blit(reward, (x + 190, row_y + 20))

        bar_rect = pygame.Rect(x + 14, row_y + 39, panel_width - 28, 7)
        pygame.draw.rect(surface, (60, 60, 70), bar_rect, border_radius=3)
        pygame.draw.rect(surface, (0, 200, 100), (bar_rect.x, bar_rect.y, int(bar_rect.w * progress_percent), bar_rect.h), border_radius=3)
        row_y += 34
...
def rebuild_changelog_cache(w, h):
    pass
...
class DamageText:
    def __init__(self, x, y, damage, color, is_crit=False):
        ...
        # 爆擊時數字停留更久，初始向上彈跳速度更快
        self.timer = 50 if is_crit else 35
        self.vel_y = -3.5 if is_crit else -2.0
        self.alpha = 255
    ...
    def update(self):
        self.y += self.vel_y
        self.vel_y += 0.2  # 加入重力讓數字有彈出並掉落的動態感，增加打擊感
        self.timer -= 1
        self.alpha = max(0, int((self.timer / 35) * 255))
...
class Chest:
    def __init__(self, x, y, c_type="NORMAL"):
        ...
        # [關鍵修復] 初始化時就綁定在「世界座標」，且後續不隨畫面滾動改變
        self.rect.center = (int(self.x), int(self.y))
    ...
    def draw(self, surface):
        ...
        # 繪圖時產生一個「暫時的矩形 (draw_rect)」來畫，絕對不要去動到 self.rect 的物理碰撞框
        draw_rect = self.rect.copy()
        draw_rect.center = (draw_x, draw_y)
...
class EnemyBullet:
    def __init__(self, x, y, dir_x, dir_y, color=ORANGE, is_homing=False, weapon=None, core_color=WHITE, style="round"):
    ...
    def draw(self, surface): 
        ...
        if self.style == "diamond":
            pts = [
                (draw_center[0], draw_center[1] - self.radius - 1),
                (draw_center[0] + self.radius + 1, draw_center[1]),
                (draw_center[0], draw_center[1] + self.radius + 1),
                (draw_center[0] - self.radius - 1, draw_center[1]),
            ]
            pygame.draw.polygon(surface, self.core_color, pts)
        elif self.style == "slash":
            direction = pygame.math.Vector2(self.dir_x, self.dir_y)
            if direction.length_squared() == 0:
                direction = pygame.math.Vector2(1, 0)
            side = direction.rotate(90)
            center = pygame.math.Vector2(draw_center)
            front = center + direction * (self.radius + 5)
            back = center - direction * (self.radius + 5)
            pygame.draw.line(surface, self.core_color, (int(front.x), int(front.y)), (int(back.x), int(back.y)), 4)
            pygame.draw.line(surface, WHITE, (int((center + side * 3).x), int((center + side * 3).y)), (int((front + side * 3).x), int((front + side * 3).y)), 2)
        else:
            pygame.draw.circle(surface, self.core_color, draw_center, max(2, self.radius // 2))
#敵人類別，包含位置、大小、生命值、攻擊方式和特殊效果等屬性，更新時會根據玩家位置來移動和攻擊，並且處理冰凍、燃燒和其他狀態效果的邏輯，繪製時會根據狀態來呈現不同的圖像或幾何圖形，確保玩家在遊戲中能夠清晰地看到敵人的位置和狀態
class Enemy:
    ...
    def draw(self, surface):
        ...
        if self.combat_type == "kamikaze":
            ...
            if anim_frames:
                ...
                # 沒圖片時的幾何閃白
                if self.hit_timer > 0: color = WHITE 
                elif self.frost_timer > 0: color = (100, 200, 255)
                if self.is_elite:
...
def _boss_x(self):
    return self.pos.x
def _boss_set_x(self, value):
    self.pos.x = value
    self.rect.center = (round(self.pos.x), round(self.pos.y))
def _boss_y(self):
    return self.pos.y
def _boss_set_y(self, value):
    self.pos.y = value
    self.rect.center = (round(self.pos.x), round(self.pos.y))

for _boss_cls in (Boss, ChargerBoss, BerserkerBoss):
    _boss_cls.x = property(_boss_x, _boss_set_x)
    _boss_cls.y = property(_boss_y, _boss_set_y)

def draw_boss_health_bar(surface, boss_obj):
    bar_rect = pygame.Rect(80, HEIGHT - 70, WIDTH - 160, 24)
    ...
    if fill_w > 0:
        ...
        highlight = pygame.Rect(fill_rect.x + 4, fill_rect.y + 4, max(0, fill_rect.width - 8), 6)
    ...
    title_txt = font.render(f"{boss_obj.name}  Lv.{boss_obj.spawn_level}{phase_label}", True, edge_color)
    hp_txt = small_font.render(f"{max(0, int(boss_obj.hp))} / {boss_obj.max_hp}", True, WHITE)
    surface.blit(title_txt, (bar_rect.x, bar_rect.y - title_txt.get_height() - 12))
    surface.blit(hp_txt, (bar_rect.right - hp_txt.get_width(), bar_rect.y - hp_txt.get_height() - 10))
    if boss_obj.state == "TRANSFORM":
        rage_txt = small_font.render("RAGE CORE REBOOTING - HP REFILL", True, RED)
        surface.blit(rage_txt, (bar_rect.centerx - rage_txt.get_width() // 2, bar_rect.y - 54))

def spawn_a5_boss():
    global boss, boss_active, boss_spawn_count, boss_warning_timer
    boss_spawn_count += 1
    player_pos = pygame.math.Vector2(player.x, player.y)
    cycle = boss_spawn_count % 3
    if cycle == 1:
        boss = Boss(player.level, player_pos)
    elif cycle == 2:
        boss = ChargerBoss(player.level, player_pos)
    else:
        boss = BerserkerBoss(player.level, player_pos)
    boss_active = True
...
def defeat_a5_boss():
    global boss_active
    if not boss or boss.state == "DEFEAT":
        return
    boss.state = "DEFEAT"
    boss.defeat_timer = 0
    ...
    for _ in range(10):
        items.append(DropItem(boss.x + random.randint(-40, 40), boss.y + random.randint(-40, 40), "SCRAP", random.randint(2, 5)))
    items.append(DropItem(boss.x, boss.y, "KEY"))
    for _ in range(50):
        particles.append(Particle(boss.x, boss.y, YELLOW))

def damage_a5_boss(amount, source_x=None, source_y=None, color=WHITE, critical=False):
    if not boss_active or not boss or boss.state == "DEFEAT":
        return False
    if hasattr(boss, "can_take_damage") and not boss.can_take_damage():
        for _ in range(5):
            particles.append(Particle(boss.x, boss.y, GRAY))
        return False
    boss.hp -= amount
    if 'task_system' in globals():
        task_system.update_progress("damage", amount)
    damage_texts.append(DamageText(source_x or boss.x, (source_y or boss.y) - 20, amount, color, critical))
    for _ in range(8):
        particles.append(Particle(boss.x, boss.y, YELLOW))
    if boss.hp <= 0 and hasattr(boss, "survive_lethal_damage") and boss.survive_lethal_damage():
        for _ in range(28):
            particles.append(Particle(boss.x, boss.y, RED))
    elif boss.hp <= 0:
        defeat_a5_boss()
    return True
chosen_upgrades = []
...
boss_warning_timer, boss_spawn_count = 0, 0
...
def enter_bunker(success=False):
    ...
    global boss_army_active, extraction_timer, extraction_pt, extract_progress, enemy_spawn_timer, boss_warning_timer
...
def start_raid():
    ...
    global boss_active, boss, player, enemy_spawn_timer, boss_warning_timer, raid_start_time, last_survival_second
    game_state = "PLAYING"
    player.x, player.y = MAP_WIDTH//2, MAP_HEIGHT//2
...
def full_wipe(mode="NORMAL"):
    global player, game_mode, chosen_upgrades, lost_item, defeated_boss_levels, boss_spawn_count, boss_warning_timer, task_system
    game_mode = mode
    player = Player()
...
#處理玩家與寶箱的碰撞，檢測玩家是否與任何寶箱發生碰撞，根據碰撞的方向來調整玩家的位置，確保玩家不會穿過寶箱並且能夠正確地與寶箱互動
...
# 地堡四大面板背景
shop_bg = pygame.Rect(WIDTH//2 - 300, HEIGHT//2 - 250, 600, 500) #黑市
stash_bg = pygame.Rect(WIDTH//2 - 380, HEIGHT//2 - 250, 760, 500) #收藏箱
mod_bg = pygame.Rect(WIDTH//2 - 380, HEIGHT//2 - 260, 760, 520) #改造台
wep_stash_bg = pygame.Rect(WIDTH//2 - 380, HEIGHT//2 - 280, 760, 560) #武器庫
# 地堡四大面板關閉按鈕 (精準對齊各自面板的右上角)
...
# 各面板的 6*4 格背包繪製起點 (x, y 座標)
s_start_x, s_start_y = WIDTH//2 - 350, HEIGHT//2 - 150       # 收藏箱面板的箱子起點
p_start_x_s, p_start_y_s = WIDTH//2 + 30, HEIGHT//2 - 150    # 收藏箱面板的玩家背包起點
p_start_x_m, p_start_y_m = WIDTH//2 - 350, HEIGHT//2 - 40    # 改造台面板的玩家背包起點
p_start_x_w, p_start_y_w = WIDTH//2 - 344, HEIGHT//2 + 115   # 武器庫面板的玩家背包起點

while running:
    reward_ready_sound = False
    if 'lost_item' in globals() and lost_item:
    ...
    if task_system.claim_completed_rewards(player):
        play_sound("levelup")
    ...
    for event in pygame.event.get():
        ...
        #主菜單界面，處理玩家在主菜單上的點擊事件，包括開始遊戲、查看更新日誌和退出遊戲等選項，確保玩家在進入遊戲前能夠有一個清晰的選擇界面
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if show_changelog:
                    ...
                    elif changelog_button.collidepoint(event.pos): show_changelog, changelog_scroll = False, 0
                else:
        ...
        #商店界面，處理玩家在商店內的交互事件，包括購買升級、關閉商店等選項
        elif game_state == "SHOP":
            #玩家在商店界面內點擊升級按鈕，首先檢查鼠標位置是否在升級按鈕的區域內，如果是，則根據玩家選擇的升級類型（生命值、傷害、速度、耐力、護盾或能量）
            #  來增加相應的屬性值，同時扣除玩家的資源（如金錢或積分），並播放升級成功的聲音效果；
            # 如果鼠標位置在關閉按鈕的區域內，則返回掩體界面；
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                if btn_hp.collidepoint(event.pos) and persistent_stats["scrap"] >= 50:
                    ...
                    task_system.update_progress("upgrade_hp", 1)
                    ...
                    task_system.update_progress("upgrade_stamina", 1)
    ...
    #遊戲主循環，根據當前的遊戲狀態更新遊戲邏輯，包括玩家移動、敵人生成和行為、子彈更新、物品交互和界面交互等，確保遊戲在不同狀態下有適當的行為和反應
        ...
        if raid_start_time:
            survival_second = (pygame.time.get_ticks() - raid_start_time) // 1000
            if survival_second > last_survival_second:
                reward_ready_sound = task_system.update_progress("survive", survival_second - last_survival_second) or reward_ready_sound
            ...
            for g in items[:]:
                if g.item_type not in ["EXP", "MAGNET", "BOMB", "SHIELD"] and math.hypot(player.x - g.x, player.y - g.y) < 70:
                    ...
                    if player.add_item(new_item):
                        items.remove(g)
                        play_sound("exp")
                        if g.item_type == "SCRAP":
                            reward_ready_sound = task_system.update_progress("collect_scrap", g.count) or reward_ready_sound
                        elif g.item_type == "KEY":
                            reward_ready_sound = task_system.update_progress("collect_key", g.count) or reward_ready_sound
        else:
            for c in chests:
        ...
        if boss_active and boss and boss.state == "ENTRANCE":
            boss.update(pygame.math.Vector2(player.x, player.y), bullets)
        ...
        if player.aura_level > 0:
            ...
            for e in enemies[:]:
                if math.sqrt((e.x - player.x)**2 + (e.y - player.y)**2) <= aura_radius:
                    ...
                    reward_ready_sound = task_system.update_progress("damage", aura_damage) or reward_ready_sound
                    ...
                    if e.hp <= 0:
                        ...
                        reward_ready_sound = task_system.update_progress("kill", 1) or reward_ready_sound
                        if was_elite:
                            reward_ready_sound = task_system.update_progress("kill_elite", 1) or reward_ready_sound
                        if e in enemies: enemies.remove(e)
            if boss_active and boss.state != "DEFEAT":
                if math.sqrt((boss.x - player.x)**2 + (boss.y - player.y)**2) <= aura_radius:
                    damage_a5_boss(aura_damage, boss.x, boss.y, BLUE, False)
        ...
        for b in bullets:
            ...
            for c in chests:
                if b.rect.colliderect(c.rect):
                    hit_chest = True
                    if not b.is_piercing: # 如果沒有穿透，打在掩體上產生碎石火花
                        for _ in range(3): particles.append(Particle(b.x, b.y, GRAY))
                        play_sound("hit")
                    break
            
            if hit_chest and not b.is_piercing:
                continue # 子彈撞到掩體且無法穿透，子彈直接銷毀不往下判定打怪
            ...
            if b.explode:
                ...
                for e in enemies[:]:
                    if math.hypot(e.x - b.x, e.y - b.y) < 120: 
                        ...
                        reward_ready_sound = task_system.update_progress("damage", actual_dmg) or reward_ready_sound
                ...
                if boss_active and boss.state != "DEFEAT" and math.hypot(boss.x - b.x, boss.y - b.y) < 150: 
                    damage_a5_boss(b.damage, b.x, b.y, ORANGE, getattr(b, "is_crit", False))
                continue 
            # 子彈與敵人碰撞檢測        
            hit_something = False
            for e in enemies[:]:
                if b.rect.colliderect(e.rect):
                    ...
                    #######################################################################################################
                    #  閃白、擊退與卡肉頓幀，為了製造視覺打擊感
                    e.hit_timer = 4  # 觸發敵人閃白 4 個 Frame
                    
                    # 物理擊退效果(依照子彈傷害計算，暴擊加倍)
                    if e.combat_type != "kamikaze": # 自爆怪太重推不動
                        kb_force = min(6.0, b.damage * 0.1) 
                        if b.is_crit: kb_force *= 1.5
                        e.x += b.dir_x * kb_force
                        e.y += b.dir_y * kb_force
                    
                    # 卡肉頓幀與強烈震動 (只有爆擊或高傷害才觸發，避免太卡)
                    if b.is_crit or b.damage >= 50:
                        screen_shake = max(screen_shake, 6) # 強烈螢幕震動
                        pygame.time.delay(12)               # 畫面凍結 12 毫秒
                    # ############################################################################################
                    ...
                    play_sound("hit")
                    reward_ready_sound = task_system.update_progress("damage", b.damage) or reward_ready_sound
            ...
            if boss_active and b.rect.colliderect(boss.rect):
                ...
                if damage_a5_boss(b.damage, boss.x, boss.y, RED if b.is_crit else WHITE, b.is_crit):
                    if b.is_crit or b.damage >= 50:
        ...
        if boss_active and boss:
            ...
            # Boss 撞擊寶箱邏輯 (會中斷致命衝鋒並產生巨大震動)
            if resolve_chest_collision(boss, chests):
                if boss.state in ["CHARGE", "DASH"]:
                    boss.state = "SHOOT" if isinstance(boss, Boss) else ("RECOVER" if isinstance(boss, ChargerBoss) else "HUNT")
                    boss.state_timer = 0
                    screen_shake = 15
                    play_sound("shoot_cannon") # 撞擊巨響
                    for _ in range(15): particles.append(Particle(boss.x, boss.y, GRAY)) # 衝撞的碎石特效
            ...
            for g in items[:]:
                ...
                if g.item_type in ["EXP", "MAGNET", "BOMB", "SHIELD"] and player.rect.colliderect(g.rect):
                    ...
                    if g.item_type == "EXP":
                        ...
                        enemies.clear()
                        if boss_active and boss.state != "DEFEAT":
                            damage_a5_boss(800, boss.x, boss.y, ORANGE, True)
                            particles.extend([Particle(boss.x, boss.y, ORANGE) for _ in range(15)])
                        play_sound("hit")
    ...
    if game_state in ["BUNKER", "SHOP", "GENERAL_STASH", "MOD_STATION", "WEAPON_STASH"]:
        ...
        draw_task_panel(screen, task_system, 20, HEIGHT - 245)
        ...
        if game_state == "SHOP":
            ...
            # 使用包含空格的字串讓 draw_hover_button 自動換行，還有我讓字體顏色反黑提升閱讀性
        ...
        #格子收藏箱界面，顯示玩家存放的物品，並根據物品類型使用不同顏色的圓點表示，同時在游標懸停時顯示物品資訊和高亮邊框在右側顯示玩家背包的內容，但禁止放置武器
        elif game_state == "GENERAL_STASH":
            screen.blit(dim_surface, (0, 0))
        ...
        #武器改造台
            ...
            for i, wep in enumerate(arsenal_weapons_list):
                col, row = i % 2, i // 2
                box = pygame.Rect(col*335 + 10, row*50 + 5, 315, 42) # 調整格子間距與大小
    ...
    # 根據遊戲狀態繪製遊戲主畫面，包含背景、地圖邊界、玩家、敵人、子彈、物品、粒子效果、傷害數字，以及根據不同狀態顯示相應的提示和UI元素，例如開箱提示、撿取提示、Boss方向箭頭等，同時在玩家死亡時顯示遺失物並提供撿取選項
        ...
        for dt in damage_texts: dt.draw(screen)
        if boss_active:
            boss.draw(screen, pygame.math.Vector2(camera_x, camera_y))
            draw_boss_direction_arrow(screen, boss, camera_x, camera_y)
        ...
        new_var = screen.blit(skill_txt, (20, HEIGHT - 40))
        ...
        if boss_active:
            ...
            if boss.state == "ENTRANCE":
                ...
                screen.blit(entrance_text, (WIDTH//2 - entrance_text.get_width()//2, HEIGHT//2 - 180))
                progress = min(1, boss.state_timer / boss.entrance_duration)
                bar_rect = pygame.Rect(WIDTH//2 - 180, HEIGHT//2 - 125, 360, 14)
                pygame.draw.rect(screen, GRAY, bar_rect, border_radius=7)
                pygame.draw.rect(screen, RED, (bar_rect.x, bar_rect.y, int(bar_rect.width * progress), bar_rect.height), border_radius=7)
            elif boss_warning_timer > 0:
                warning_txt = font.render(f"警告：{boss.name} 出現！準備戰鬥！", True, RED)
                screen.blit(warning_txt, (WIDTH//2 - warning_txt.get_width()//2, HEIGHT - 110))
            else:
                state_message, state_color = boss.get_state_message()
                state_txt = font.render(state_message, True, state_color)
                screen.blit(state_txt, (WIDTH//2 - state_txt.get_width()//2, HEIGHT - 110))
    ...
    if game_state == "MENU":
        ...
        title = large_font.render("太空戰役", True, BLUE)
