<<<<<<< HEAD
"""
整合 B4 開放世界/防穿透核心 + A3 完整 UI 系統
- 移除 ctypes 與輸入法強制切換
- A3 主選單、難易度選擇、更新日誌、暫停紀錄與升級確認介面
- 保留 B4 的 12 種武器切換、能量技能、衝刺機制、Boss 特性
"""
...
class Bullet:
    def __init__(self, x, y, target_x, target_y, weapon):
        ...
        self.b_type, self.damage = weapon.bullet_type, weapon.damage
...
class Enemy:
    ...
    def update(self, target_x, target_y, all_enemies):
        ...
        if dist > min_p_dist:
            ...
            if dist > 0:
                self.x += ((self.x - target_x) / dist) * (current_speed * 0.8)
                self.y += ((self.y - target_y) / dist) * (current_speed * 0.8)
    ...
    def draw(self, surface):
        ...
        if anim_frames:
            ...
            if self.is_elite:
                ...
                glow_rect = draw_rect.copy().inflate(glow, glow)
...
class DropItem:
    ...
    def update(self, p_x, p_y, mag_rad):
        ...
        if dist < mag_rad and dist > 0:
            self.x += ((p_x - self.x) / dist) * 8 
            self.y += ((p_y - self.y) / dist) * 8 
        self.rect.center = (int(self.x), int(self.y))
...
# UI 與遊戲狀態相關
upgrade_options =[
    {"title": "生命躍升", "desc": ["最大血量 +50", "並恢復當前血量"], "type": "life", "weight": 2},
    {"title": "體能強化", "desc":["最大體力 +50", "衝刺次數增加"], "type": "support", "weight": 2},
    {"title": "能量擴容", "desc":["最大能量 +50", "施放更多技能"], "type": "attack", "weight": 2},
    {"title": "裝甲模組", "desc":["最大護盾 +25", "提升生存能力"], "type": "life", "weight": 1},
    {"title": "輕量推進", "desc":["基礎移動速度提升", "走位更靈活"], "type": "support", "weight": 1},
    {"title": "磁吸核心", "desc":["經驗吸收範圍提升", "更容易拾取掉落物"], "type": "support", "weight": 1}
]

# 按鈕與視窗定義 (依 1024x768 調整)
...
def rebuild_changelog_cache(content_width, content_height):
    ...
    CHANGELOG = [
        "v1.4 - 整合版本",
        "- B4 開放世界地圖與 12 種武器庫完美保留",
        "- 敵人不再重疊穿透玩家，具有排斥力",
        "- 導入 A3 絢麗的 UI 選單與升級卡牌介面",
        "- UI 畫面與按鈕已自適應 1024x768 視窗",
        "- 強化選單新增「確認選擇」並根據類型上色",
        "- 暫停選單加入升級歷史紀錄",
        "v1.3 - 先前版本",
        "- 加入開放世界鏡頭跟隨功能",
...
def draw_boss_direction_arrow(surface, boss_obj, cam_x, cam_y):
    if not boss_obj or (hasattr(boss_obj, "state") and boss_obj.state == "DEFEAT"):
        return
    boss_screen_x = boss_obj.x - cam_x
    boss_screen_y = boss_obj.y - cam_y
    if 0 <= boss_screen_x <= WIDTH and 0 <= boss_screen_y <= HEIGHT:
        return
...
def choose_upgrade_cards():
    ...
    available = list(range(len(upgrade_options)))
    current_upgrade_choices = []
...
def apply_upgrade(choice):
    ...
    elif choice == 3: player.max_shield += 25; player.shield += 25
    elif choice == 4: player.base_speed += 1.0
    elif choice == 5: player.magnet_radius += 40
...
while running:
    ...
    if game_state == "PLAYING":
        ...
        if mouse_btns[0] and shoot_cooldown <= 0 and not player.is_dashing:
            if current_wep.bullet_type == "shotgun":
                for i in range(-2, 3):
                    angle = math.atan2(world_mouse_y - player.y, world_mouse_x - player.x) + math.radians(i * 12)
                    bullets.append(Bullet(player.rect.centerx, player.rect.centery, player.x + math.cos(angle)*10, player.y + math.sin(angle)*10, current_wep))
            elif current_wep.bullet_type == "flamethrower":
                bullets.append(Bullet(player.rect.centerx, player.rect.centery, world_mouse_x + random.randint(-40, 40), world_mouse_y + random.randint(-40, 40), current_wep))
            else:
                bullets.append(Bullet(player.rect.centerx, player.rect.centery, world_mouse_x, world_mouse_y, current_wep))
            ...
            for e in enemies[:]:
                if b.rect.colliderect(e.rect):
                    ...
                    if e.hp <= 0:
                        ...
                        if e.is_elite: 
                            items.append(DropItem(e.x-10, e.y, "EXP")); items.append(DropItem(e.x+10, e.y, "EXP"))
                        else:
                            if random.random() < 0.4: items.append(DropItem(e.x, e.y, "EXP"))
                        enemies.remove(e)
        ...
        # 吃經驗
        for g in items[:]:
            g.update(player.x, player.y, player.magnet_radius)
    ...
    if game_state in ["PLAYING", "PAUSED", "LEVEL_UP", "GAME_OVER"]:
        ...
        screen.blit(font.render("體力 (Q鍵衝刺)", True, WHITE), (180, 62)) 
        ...
        screen.blit(font.render(weapon_str, True, WHITE if game_mode == "NORMAL" else RED), (20, 115))
    ...
    if game_state == "MENU":
        ...
        subtitle = font.render("A3 視覺重製版", True, WHITE)
        ...
        controls = ["移動: WASD", "射擊: 滑鼠左鍵  |  技能: 滑鼠右鍵", "衝刺: SPACE 鍵 / Q 鍵", "切換武器: E 鍵", "暫停: ESC 鍵"]
        ...
        screen.blit(small_font.render("v1.4", True, GRAY), (WIDTH - 40, HEIGHT - 25))
        ...
        subtitle = font.render("挑戰模式下敵人強度大幅提升，請做好準備", True, GRAY)
        ...
        for i, line in enumerate(["基礎倍率：1.0x", "適合新手體驗全部 12 種武器", "穩定探索地圖邊界"]):
        ...
        for i, line in enumerate(["難度倍率：1.75x", "走位失誤極易被怪物浪潮淹沒", "考驗高頻率衝刺與火力切換"]):
