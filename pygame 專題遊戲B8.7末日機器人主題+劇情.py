...
def load_sound(name, filename):
    try:
        ...
        if os.path.exists(path): sounds[name] = pygame.mixer.Sound(path); sounds[name].set_volume(0.5)
...
def load_weapon_sound(wk, fn, fb):
    ...
    if sounds.get(wk): sounds[wk].set_volume(0.08)
...
if os.path.exists(bgm_path): pygame.mixer.music.load(bgm_path); pygame.mixer.music.set_volume(0.4); pygame.mixer.music.play(loops=-1)
