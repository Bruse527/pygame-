...
class ChargerBoss:
    ...
    def draw(self, surface, camera):
        ...
        if self.state == "AIM":
            start = pygame.math.Vector2(cx, cy) + direction * 35
            end = pygame.math.Vector2(cx, cy) + direction * 360
            side = direction.rotate(90)
            pygame.draw.line(surface, YELLOW, (int(start.x), int(start.y)), (int(end.x), int(end.y)), 3)
            pygame.draw.line(surface, ORANGE, (int((start + side * 24).x), int((start + side * 24).y)), (int((end + side * 24).x), int((end + side * 24).y)), 2)
            pygame.draw.line(surface, ORANGE, (int((start - side * 24).x), int((start - side * 24).y)), (int((end - side * 24).x), int((end - side * 24).y)), 2)
            for i in range(5):
                mark = start.lerp(end, 0.18 + i * 0.16)
                arrow_back = mark - direction * 16
                pygame.draw.line(surface, WHITE, (int(mark.x), int(mark.y)), (int((arrow_back + side * 10).x), int((arrow_back + side * 10).y)), 2)
                pygame.draw.line(surface, WHITE, (int(mark.x), int(mark.y)), (int((arrow_back - side * 10).x), int((arrow_back - side * 10).y)), 2)
        elif self.state == "DASH":
...
while running:
        ...
        for g in gems[:]:
            ...
            if player.rect.colliderect(g.rect):
                ...
                if player.exp >= player.max_exp:
                    player.level += 1; player.exp = 0; player.max_exp = int(player.max_exp * 1.0)
    ...
    if game_state == "LEVEL_UP":
        ...
        subtitle = font.render("生存、升級、擊敗 Boss", True, WHITE)
        ...
        version_txt = font.render("v1.377", True, GRAY)
