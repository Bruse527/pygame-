...
class Player:
    ...
    def update(self, clamp_rect=None):
        ...
        # 紀錄實際動量供傾斜使用
        self.vel_x = move_x * self.base_speed
        self.vel_y = move_y * self.base_speed
        ...
        if self.is_dashing:
            ...
            self.z = math.sin(progress * math.pi) * 30
    ...
    def draw(self, surface, current_wep=None):
        draw_player = True
        # 繪圖時扣除 Z 軸高度，產生跳離地面錯覺
        draw_center = (int(self.rect.centerx - camera_x), int(self.rect.centery - camera_y - self.z))
        ...
        if draw_player:
            ...
            if anim_frames:
                ...
                # 呼吸擠壓和慣性傾斜
                breath = 1.0 + math.sin(pygame.time.get_ticks() * 0.008) * 0.05
                tilt_angle = -self.vel_x * 2.0  # 移動時身體嚴重傾斜
                
                if self.is_dashing:
                    tilt_angle = -self.dash_dir_x * 35 # 衝刺時傾斜 35 度！
                    scaled_img = pygame.transform.scale(img, (int(self.size * 1.3), int(self.size * 0.7))) # 空中拉長
                else:
                    scaled_img = pygame.transform.scale(img, (int(self.size), int(self.size * breath)))
            ...
            if self.stamina < self.dash_cost: 
                pygame.draw.circle(surface, GRAY, draw_center, self.size, 2)

            if current_wep:
                # 武器跟著 Z 軸浮空
                mx, my = pygame.mouse.get_pos(); dx, dy = (mx + camera_x) - self.x, (my + camera_y) - (self.y - self.z)
                ...
                if gun_img:
                    ...
                    end_x, end_y = draw_center[0] + dir_x * 25, draw_center[1] + dir_y * 25
                    wep_color = YELLOW
                    ...
                    pygame.draw.line(surface, GRAY, draw_center, (end_x, end_y), 6)
        ...
        if self.aura_level > 0:
            ...
            pygame.draw.circle(surface, (0, 180, 255), (draw_center[0], draw_center[1] + self.z), 95 + self.aura_level * 25 + pulse, 2)
            
        if self.drone_level > 0:
            #  無人機加上獨立上下漂浮
            drone_hover = math.sin(pygame.time.get_ticks() * 0.008) * 15
            drone_x, drone_y = draw_center[0] + math.cos(self.drone_angle) * 55, draw_center[1] + math.sin(self.drone_angle) * 55 + drone_hover
...
class Enemy:
    ...
    def update(self, target_x, target_y, all_enemies, enemy_bullets):
        ...
        self.rect.center = (int(self.x), int(self.y))
    #繪製敵人的圖像，首先根據狀態來決定使用哪個圖像或像素圖形來繪製敵人，然後根據位置和攝像機偏移來計算繪製位置，最後將圖像或幾何圖形繪製到屏幕上，確保玩家在遊戲中能夠清晰地看到敵人的位置和狀態
    def draw(self, surface):
        # 怪物移動時產生生動的上下彈跳與傾斜
        is_moving = abs(self.dir_x) > 0.1 or abs(self.dir_y) > 0.1
        self.z = abs(math.sin(pygame.time.get_ticks() * 0.015)) * 12 if is_moving else 0
        tilt_angle = -self.dir_x * (self.speed * 4) # 根據移動方向大幅傾斜
        ...
        if self.combat_type == "kamikaze":
            ...
            if anim_frames:
                ...
                # 套用旋轉傾斜
                final_img = pygame.transform.rotate(img, tilt_angle)
                surface.blit(final_img, final_img.get_rect(center=draw_center))
            else:
                color = (150, 0, 150) if self.is_elite else RED
                if getattr(self, 'hit_timer', 0) > 0: color = WHITE 
        ...
        if self.max_shield > 0 and self.shield > 0:
            pygame.draw.rect(surface, GRAY, (draw_rect.x, draw_rect.y + self.z - 14, self.size, 4))
            pygame.draw.rect(surface, (0, 150, 255), (draw_rect.x, draw_rect.y + self.z - 14, self.size * (max(0, self.shield)/self.max_shield), 4))
        if self.hp < self.max_hp:
            pygame.draw.rect(surface, GRAY, (draw_rect.x, draw_rect.y + self.z - 8, self.size, 4))
            pygame.draw.rect(surface, GREEN, (draw_rect.x, draw_rect.y + self.z - 8, self.size * (max(0, self.hp)/self.max_hp), 4))
...
# 3D 視覺 初始化前景視差粒子(讓遊戲看起來有距離感) ( Z 軸深度感)
foreground_dust = [(random.randint(0, MAP_WIDTH), random.randint(0, MAP_HEIGHT), random.uniform(1.3, 2.0), random.uniform(2, 6)) for _ in range(80)]



#設置了一個布爾變量running為True，這個變量用於控制遊戲主循環的運行狀態。
...
while running:
    ...
    #遊戲主循環，根據當前的遊戲狀態更新遊戲邏輯，包括玩家移動、敵人生成和行為、子彈更新、物品交互和界面交互等，確保遊戲在不同狀態下有適當的行為和反應
        ...
        # 3D 視覺的鏡頭探視或著你要說窺看，反正就是讓相機往滑鼠方向稍微偏移 20% (待測試)視角探頭的立體感
        mouse_x, mouse_y = pygame.mouse.get_pos()
        base_cam_x = player.x - WIDTH / 2 + (mouse_x - WIDTH / 2) * 0.2
        base_cam_y = player.y - HEIGHT / 2 + (mouse_y - HEIGHT / 2) * 0.2

        camera_x = max(0, min(MAP_WIDTH - WIDTH, base_cam_x)) + shake_x
        camera_y = max(0, min(MAP_HEIGHT - HEIGHT, base_cam_y)) + shake_y
        ...
        for b in bullets:
            ...
            if boss_active and b.rect.colliderect(boss.rect):
                ...
                if boss.b_type == "YELLOW" and boss.state == "EVADE":
                    ...
                    if b.is_vampiric and random.random() < 0.05: player.hp = min(player.max_hp, player.hp + 2)
                    boss.hp -= b.damage
                    # Boss 專屬閃白與卡肉 (目前Boss不吃擊退 暫時不更改)
                    boss.hit_timer = 4
                    if b.is_crit or b.damage >= 50:
                        screen_shake = max(screen_shake, 8) # Boss 震動更強烈
                        pygame.time.delay(15)               # 畫面凍結 15 毫秒
    ...
    # 根據遊戲狀態繪製遊戲主畫面，包含背景、地圖邊界、玩家、敵人、子彈、物品、粒子效果、傷害數字，以及根據不同狀態顯示相應的提示和UI元素，例如開箱提示、撿取提示、Boss方向箭頭等，同時在玩家死亡時顯示遺失物並提供撿取選項
        ...
        # 將背景 Y 軸壓縮 50% 來模擬 45 度視角傾斜透視 ，這是我參考的 Mode 7 想出來的，簡單來講就像是馬力歐賽車第一版
        if images.get("bg"):
            bg_w = WIDTH
            bg_h = int(HEIGHT * 0.5) # [3D 視覺] Y 軸壓縮一半
            tilted_bg = pygame.transform.scale(images["bg"], (bg_w, bg_h))
            for x in range(0, MAP_WIDTH, bg_w):
                for y in range(0, MAP_HEIGHT, bg_h):
                    ...
                    if draw_x + bg_w > 0 and draw_x < WIDTH and draw_y + bg_h > 0 and draw_y < HEIGHT: 
                        screen.blit(tilted_bg, (draw_x, draw_y))
        else:
            # 沒背景圖時，會畫出帶有透視感的科技網格
            grid_w, grid_h = 100, 50 # 寬 100，高 50 (45度視角) 可能要再調整
            off_x, off_y = -int(camera_x) % grid_w, -int(camera_y) % grid_h
            for x in range(off_x - grid_w, WIDTH, grid_w): pygame.draw.line(screen, (20, 25, 30), (x, 0), (x, HEIGHT), 2)
            for y in range(off_y - grid_h, HEIGHT, grid_h): pygame.draw.line(screen, (20, 25, 30), (0, y), (WIDTH, y), 2)

        # 繪製 3D 地圖邊界牆壁 (北面的牆)
        map_draw_x, map_draw_y = -int(camera_x), -int(camera_y)
        wall_height = 250
        north_wall = pygame.Rect(map_draw_x, map_draw_y - wall_height, MAP_WIDTH, wall_height)
        pygame.draw.rect(screen, (15, 18, 25), north_wall) # 牆壁主體
        pygame.draw.rect(screen, (30, 35, 45), north_wall, 4) # 牆壁頂緣
        pygame.draw.line(screen, CYAN, (north_wall.left, north_wall.bottom), (north_wall.right, north_wall.bottom), 4) # 發光的牆角地平線
        pygame.draw.rect(screen, RED, (map_draw_x, map_draw_y, MAP_WIDTH, MAP_HEIGHT), 5) # 地板紅色警示邊線
        #############################################################
        
        # 讓遊戲視覺更好更3D ㄒ Y-Sorting + 動態陰影
        # 收集所有在地板上的實體
        render_entities = []
        render_entities.extend(chests)
        render_entities.extend(items)
        render_entities.extend(enemies)
        if lost_item: render_entities.append(lost_item)
        if boss_active and boss: render_entities.append(boss)
        if game_state != "DIED": render_entities.append(player)

        # 深度排序 (Y-Sorting) 產生 3D 遮擋感 2.5D 的視覺效果，y軸越大代表越靠近玩家，畫在上面
        # y軸越小代表越遠離玩家，畫在下面，這樣玩家就會被前面的敵人或物品遮住增加空間感
        # 依照 Y 座標由小到大排序 (上面的先畫，下面的後畫，前面的東西會遮住後面的東西)
        render_entities.sort(key=lambda obj: obj.y)

        # 繪製統一的立體腳底陰影 (將 2D 貼圖「立」在 3D 平面上)
        floor_surf = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        for obj in render_entities:
            # 獲取物件的寬度與高度，用來動態計算陰影大小
            obj_w = obj.rect.width if hasattr(obj, 'rect') else 30
            obj_h = obj.rect.height if hasattr(obj, 'rect') else 30
            draw_x = int(obj.x - camera_x)
            draw_y = int(obj.y - camera_y)
            
            # 計算物件中心與畫面中心的距離，讓陰影根據物件在畫面中的位置產生偏移，模擬光源角度變化帶來的陰影位移效果，動態透視廣角陰影
            # 動態透視廣角陰影
            center_dx, center_dy = draw_x - (WIDTH / 2), draw_y - (HEIGHT / 2)
            shadow_offset_x = center_dx * 0.15  
            shadow_offset_y = center_dy * 0.15 + (obj_h * 0.4) 
            # 陰影會因為物體浮空而等比例縮小
            shadow_scale = max(0.3, 1.0 - (z_height / 40.0))
            shadow_rect = pygame.Rect(0, 0, obj_w * 0.85 * shadow_scale, obj_w * 0.35 * shadow_scale)
            shadow_rect.center = (draw_x + shadow_offset_x, draw_y + shadow_offset_y)
            pygame.draw.ellipse(floor_surf, (0, 0, 0, 110), shadow_rect)
            # 根據玩家和Boss的動畫幀數，繪製動態的倒影在地板上，增加反射的視覺效果，利用 z_height 動態調整倒影的位置來模擬跳離水面的分離錯覺
            if obj == player or (boss_active and obj == boss):
                anim_key = "player" if obj == player else "boss_" + obj.b_type
                anim_frames = animations.get(anim_key)
                if anim_frames:
                    img = anim_frames[int(pygame.time.get_ticks() / 150) % len(anim_frames)]
                    ref_img = pygame.transform.flip(img, False, True)
                    ref_img = pygame.transform.scale(ref_img, (img.get_width(), int(img.get_height() * 0.4)))
                    ref_img.fill((255, 255, 255, 60), special_flags=pygame.BLEND_RGBA_MULT)
                    
                    # 倒影 Y 軸加上 z_height 的兩倍距離，完美模擬跳離水面的分離錯覺
                    ref_y = draw_y + obj_h * 0.6 + z_height * 1.5 
                    floor_surf.blit(ref_img, ref_img.get_rect(center=(draw_x, ref_y)))
        ...
        for obj in render_entities:
            if obj == player:
                ...
                lost_item.draw(screen); draw_lost_item_arrow(screen, camera_x, camera_y)
            else:
                obj.draw(screen)
        ...
        for dt in damage_texts: dt.draw(screen)
        if boss_active: draw_boss_direction_arrow(screen, boss, camera_x, camera_y)
        
        # 前景視差像素粒子 目前移速比相機快 1.8 倍
        fg_surf = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        for fx, fy, fspeed, fsize in foreground_dust:
            draw_fx = (fx - camera_x * fspeed * 1.8) % WIDTH
            draw_fy = (fy - camera_y * fspeed * 1.8) % HEIGHT
            pygame.draw.circle(fg_surf, (150, 200, 255, 40), (int(draw_fx), int(draw_fy)), int(fsize))
