...
pygame.init()
pygame.mixer.init()
pygame.mixer.set_num_channels(64)  # 64個音效通道
...
pygame.mouse.set_visible(False)  #隱藏系統windows預設滑鼠游標
...
def toggle_fullscreen():
    global is_fullscreen, real_screen
    is_fullscreen = not is_fullscreen
    if is_fullscreen:
        info = pygame.display.Info()
        real_screen = pygame.display.set_mode((info.current_w, info.current_h), pygame.FULLSCREEN)
    else:
        real_screen = pygame.display.set_mode((BASE_WIDTH, BASE_HEIGHT), pygame.RESIZABLE)
...
load_image("crosshair", "crosshair.png", (32, 32))  # 準心圖片
...
load_image("bullet_normal", "bullet_normal.png", (16, 16))  # 普通子彈圖示
load_image("bullet_piercing", "bullet_piercing.png", (20, 20))  # 穿甲子彈圖示
load_image("bullet_shotgun", "bullet_shotgun.png", (16, 16))  # 散彈槍子彈圖示
load_image("bullet_flamethrower", "bullet_flame.png", (30, 30))  # 火焰噴射器子彈圖示
load_image("bullet_laser", "bullet_laser.png", (10, 40))  # 雷射子彈圖示
load_image("bullet_cannon", "bullet_cannon.png", (40, 40))  # 大炮砲彈圖示
load_image("bullet_frost", "bullet_frost.png", (20, 20))  # 冰霜子彈圖示
load_image("bullet_flame_grenade", "bullet_grenade.png", (24, 24))  # 火焰手榴彈圖示
...
load_sound("levelup", "levelup.wav")  # 升級音效
load_sound("hurt", "hurt.wav")  # 受傷音效
load_sound("boss_bgm", "boss.wav")  # Boss 戰背景音樂
load_sound("gameover", "gameover.wav")  # 遊戲結束音效
load_sound("exp", "exp.wav")  # 獲得經驗值音效
load_sound("shoot_normal", "shoot_normal.wav")  # 普通射擊音效
load_sound("shoot_laser", "shoot_laser.wav")  # 雷射射擊音效
load_sound("shoot_shotgun", "shoot_shotgun.wav")  # 散彈槍射擊音效
load_sound("shoot_cannon", "shoot_cannon.wav")  # 大炮發射音效
load_sound("shoot_flame", "shoot_flame.wav")  # 火焰噴射器發射音效

#以下兩個子彈打空時會觸發
load_sound("empty_click", "empty.wav") # 卡彈聲
load_sound("reload", "reload.wav")     # 換彈聲
...
load_weapon_sound("snd_mg", "machinegun.wav", "shoot_normal")  # 機槍射擊音效
...
# 音效播放、停止函式
sound_last_played = {}  # 紀錄音效最後播放時間

def play_sound(name, loop=0, custom_cooldown=None):
    if not sounds.get(name): return
    
    now = pygame.time.get_ticks()
    # 自動為容易重疊的音效設定微冷卻 (毫秒)，防止瞬間爆音
    cooldowns = {"hit": 40, "shoot_cannon": 60, "exp": 20, "empty_click": 250}
    cooldown = custom_cooldown if custom_cooldown is not None else cooldowns.get(name, 0)
    
    if cooldown > 0:
        if name in sound_last_played and now - sound_last_played[name] < cooldown:
            return  # 如果在冷卻時間內直接攔截不播放
...
# 繪製 Boss 方向箭頭，當 Boss 在視野外時顯示指向 Boss 的箭頭，根據距離調整大小與顏色
...
# 繪製 Boss 血條，根據 Boss 狀態改變顏色與特效，並顯示名稱、等級與血量
...
# 繪製已選強化摘要面板，顯示最近選擇的強化項目與總數量，超過上限則顯示隱藏提示
...
# 繪製任務面板，根據是否在戰鬥中顯示不同的資訊量與樣式
def draw_task_panel(surface, task_system, x, y, compact=False):
    # 戰鬥中的極簡迷你模式
    if compact:
        if not task_system.current_task: return
        ...
        mini_surf = pygame.Surface((180, 45), pygame.SRCALPHA)
        pygame.draw.rect(mini_surf, (20, 24, 32, 150), mini_surf.get_rect(), border_radius=6)
        pygame.draw.rect(mini_surf, (100, 200, 100, 150), mini_surf.get_rect(), 1, border_radius=6)
        surface.blit(mini_surf, (x - 10, y - 5))
        
        # 任務標題
        surface.blit(tiny_font.render(f"目標: {task.name}", True, YELLOW), (x, y - 2))
        ...
        # 進度數字
        prog_txt = tiny_font.render(f"{int(task.current_progress)}/{task.objective_value}", True, WHITE)
...
# 重建更新與修復日誌的快取，確保在遊戲內部能夠即時反映最新的改動內容
def rebuild_changelog_cache(w, h): pass
# 繪製更新與修復日誌的彈出面板，顯示最新版本的改動內容與修復項目
...
# 繪製暫停選單中已獲得強化的總結面板，顯示玩家目前的強化列表與數量
    ...
    for _ in range(num_items):
        ...
        if rand_val < 0.2:
            ...
            if rarity_roll < 0.6:
                ...
                rarity = "金"
            wep = generate_weapon(random.choice(list(WEAPON_TYPES.keys())), rarity)
...
class DialogueManager:
    ...
    def next_line(self):
        ...
        if self.index >= len(self.current_script):
            self.active = False
            global game_state
            game_state = self.previous_state  # 對話結束，切回原本的狀態
...
STORY_SCRIPTS = {
    "opening_narrative": [
        ...
        ("系統 AI", "請利用周圍設備強化武裝。準備好後，從上方的閘門前往地表吧，我們別無選擇了。")
    ],
    "timeout_warning": [
        ("系統 AI", "警告：撤離時間已逾時！"),
...
dialogue_sys = DialogueManager()
# 繪製對話框，根據當前劇本的說話者和內容顯示在畫面下方
def draw_dialogue_box(surface):
    if not dialogue_sys.active or not dialogue_sys.current_script: return
    speaker, text = dialogue_sys.current_script[dialogue_sys.index]
    box_rect = pygame.Rect(int(WIDTH * 0.1), int(HEIGHT - 220), int(WIDTH * 0.8), int(160))
    ...
    surface.blit(panel, (box_rect.x, box_rect.y))
    name_surf = large_font.render(speaker, True, (255, 200, 50))
    surface.blit(name_surf, (box_rect.x + 20, box_rect.y + 15))
    max_w, line_y, current_line = box_rect.width - 40, box_rect.y + 65, ""
    for char in text:
        if font.size(current_line + char)[0] < max_w: current_line += char
        else:
            surface.blit(font.render(current_line, True, WHITE), (box_rect.x + 20, line_y))
            current_line = char; line_y += 30
    if current_line: surface.blit(font.render(current_line, True, WHITE), (box_rect.x + 20, line_y))
    prompt = small_font.render("點擊左鍵 或 空白鍵繼續...", True, GRAY)
    surface.blit(prompt, (box_rect.right - prompt.get_width() - 15, box_rect.bottom - 25))
...
# 根據權重隨機選擇升級選項，確保挑戰限定的選項只在挑戰模式下出現，並更新當前可選擇的升級列表和選擇狀態
...
# 根據選擇的升級選項，應用對玩家狀態的具體增益效果，並更新已選擇的升級列表
def apply_upgrade(idx, silent=False):
    ...
    if t == "生命躍升":
        ...
        player.shoot_delay_reduction += 2
    elif t == "能量飲料":
        player.stamina_regen += 0.2
...
class Weapon:
    ...
    @property # 武器的完整名稱會根據稀有度和附加屬性動態生成，例如金手槍 (速射、穿透)
...
def apply_weapon_stats(w):
    ...
    if w.rarity == "白":
        w.damage = int(base.damage * 1.0)
    elif w.rarity == "藍":
        w.damage = int(base.damage * 1.3)
    elif w.rarity == "紫":
        w.damage = int(base.damage * 1.6)
    elif w.rarity == "金":
        w.damage = int(base.damage * 2.2)
    else:
        w.damage = base.damage
    if "速射" in w.affixes:
        w.shoot_delay = max(2, int(base.shoot_delay * 0.60))
    else:
        w.shoot_delay = base.shoot_delay
    if "散射" in w.affixes:
        w.bullet_count = 3    
    else:
        w.bullet_count = 1
# 根據武器類型、稀有度和隨機附加屬性生成武器的函式，稀有度越高，附加屬性越多，附加屬性會根據武器類型過濾掉不適用的選項
def generate_weapon(base_name, rarity="白"):
    ...
    if base.bullet_type not in ["flamethrower", "flame_grenade"]:
        pool.append("燃燒")
    if c > 0:
        w.affixes = random.sample(pool, min(c, len(pool)))
    else:
        w.affixes = []
    apply_weapon_stats(w)
    return w
# 將武器庫中的武器按照類型、稀有度和附加屬性排序，方便玩家瀏覽和管理，稀有度高的武器會排在前面，同類型的武器會根據附加屬性數量和字母順序進行次級排序
def sort_weapon_stash():
    order = list(WEAPON_TYPES.keys())
    rarity_rank = {"白":0, "藍":1, "紫":2, "金":3}
    def weapon_sort_key(w):
        if w.base_name in order:
            name_index = order.index(w.base_name)
        else:
            name_index = 99
        rarity_value = rarity_rank.get(w.rarity, 0)
        affix_text = "".join(sorted(w.affixes))
        return (name_index, -rarity_value, -len(w.affixes), affix_text)
    persistent_stats["weapon_stash"].sort(key=weapon_sort_key)
# 根據物品類型和屬性計算出售價格的函式，武器價格根據稀有度和附加屬性增加，消耗品價格根據數量計算
def get_sell_value(item):
    if not item:
        return 0
    if item.type == "WEAPON":
        if item.weapon_obj.rarity == "白":
            return 20
        elif item.weapon_obj.rarity == "藍":
            return 50
        elif item.weapon_obj.rarity == "紫":
            return 120
        elif item.weapon_obj.rarity == "金":
            return 300
        else:
            return 10
    elif item.type == "MED":
...
# 快速轉移物品的函式，優先嘗試堆疊同類型的物品，最後才放入空格子，武器類型不會堆疊(為了方便區分)
...
# 掉落物品類別，包含經驗、醫療包、磁鐵、炸彈、武器等，會被玩家吸引並移動，並在畫面上有不同的繪製方式
class DropItem:
    ...
    def update(self, p_x, p_y, mag_rad):
        # 移除限制，讓所有物品（包含武器、廢料）靠近時都會被玩家或磁鐵吸過去！
...
# 撤離點類別，當玩家完成最後一個任務後會在地圖上生成一個撤離點，玩家需要前往該點並停留幾秒鐘才能完成遊戲，撤離點會有一個動態的光環效果
...
# 假人靶子類別，主要用於測試和展示玩家的傷害輸出，會在被攻擊時記錄傷害並顯示DPS，並有一個震動效果來模擬被擊中的感覺
class DummyTarget:
...
# 玩家類別，包含玩家的屬性、武器、背包、經驗和等級等資訊，提供移動、攻擊、使用物品和升級等功能，並在更新時處理衝刺、技能冷卻、回血和其他狀態效果
class Player:
    def __init__(self):
    ...
    @property # 計算玩家目前的廢料數量，遍歷背包中的物品並累加所有廢料的數量，返回總和
    ...
    def use_med(self):
        ...
        return False
    # 更新玩家狀態處理移動、衝刺、技能冷卻、回血和其他效果，並確保玩家不會超出地圖邊界或穿牆    
...
class Enemy:
    def __init__(self, level, is_elite=False, spawn_x=MAP_WIDTH/2, spawn_y=MAP_HEIGHT/2):
        ...
        if is_elite:
            ...
            if rand_val < 0.45:
                ...
                self.combat_type = "ranged"
            else:
                self.combat_type = "kamikaze"
        ...
        if self.combat_type == "kamikaze":
            ...
            if self.weapon:
                self.shoot_cd = getattr(self.weapon, "shoot_delay", 20) * 3 + random.randint(20, 60)
            else:
                self.shoot_cd = 120
        else:
            self.weapon = None
   ...
   # 更新函數處理敵人的行為邏輯，包括移動、攻擊、狀態效果和碰撞等，根據不同的戰鬥類型有不同的行為模式，並且會避免與其他敵人重疊     
    def update(self, target_x, target_y, all_enemies, enemy_bullets):
        ...
        self.x, self.y = max(0, min(self.x, MAP_WIDTH)), max(0, min(self.y, MAP_HEIGHT))
        self.rect.center = (int(self.x), int(self.y))
    # 繪製函數根據敵人的狀態和類型繪製不同的外觀效果，例如精英敵人會有特殊的光環，近戰敵人會有揮舞武器的動畫，突擊敵人會有旋轉的線條等，並且會顯示血條和護盾條    
    def draw(self, surface):
        draw_center = (int(self.rect.centerx - camera_x), int(self.rect.centery - camera_y))
        draw_rect = self.rect.copy(); draw_rect.center = draw_center
...
# 核心 Boss 系統，會在特定等級生成，具有多階段的戰鬥行為和獨特的攻擊模式，並在更新時處理狀態轉換、移動、攻擊、受傷和其他邏輯
class CoreBoss:
    ...
    # 根據當前狀態和玩家位置，更新核心的行為和位置，並處理狀態轉換、攻擊生成、受傷效果和其他邏輯
    ...
    # 根據當前狀態和階段，生成不同的攻擊模式和子彈，並將它們添加到敵人子彈列表中
    ...
    # 根據當前狀態和階段，繪製不同的外觀和特效，例如核心的形狀、顏色、光暈、子彈的樣式和其他視覺元素
    def draw(self, surface):
        ...
        anim_frames = animations.get("boss_yellow") # 讀取第一階Boss圖片
        ...
        if self.state == "ENTRANCE":
            ...
            for i in range(3):
                ...
                if alpha_val > 0: pygame.draw.circle(surface, WHITE, (cx, cy), ring_size, 2)
            ...
            for i in range(8):
                ...
                pygame.draw.circle(surface, YELLOW, (int(cx + math.cos(angle) * (self.size + 30)), int(cy + math.sin(angle) * (self.size + 30))), 3)
        elif self.state == "DEFEAT":
            progress = min(1, self.defeat_timer / 60)
            for i in range(7):
                radius = int(self.size * 0.6 + progress * 150 + i * 14)
                pygame.draw.circle(surface, (255, max(60, 210 - i * 22), 30 + i * 18), (cx, cy), radius, 3)
            core_size = max(1, int(self.size * (1 - progress * 0.85)))
            core_points = [(int(cx + math.cos(self.defeat_timer * 0.08 + i * math.pi / 4) * (core_size + (14 if i % 2 == 0 else 2))), int(cy + math.sin(self.defeat_timer * 0.08 + i * math.pi / 4) * (core_size + (14 if i % 2 == 0 else 2)))) for i in range(8)]
            ...
            for i in range(burst):
                ...
                pygame.draw.circle(surface, RED if i % 2 else YELLOW, (int(cx + math.cos(angle) * distance), int(cy + math.sin(angle) * distance)), 5)
        else:
            aura_radius = self.size + 28 + int(abs(math.sin(self.state_timer * 0.08)) * 12)
            pygame.draw.circle(surface, RED if self.state == "CHARGE" else PURPLE, (cx, cy), aura_radius, 2)
...
class BerserkerBoss:
    def __init__(self, spawn_level=5, player_x=0, player_y=0):
        ...
        self.frost_timer, self.burn_timer, self.hit_timer = 0, 0, 0
    # 更新函數，處理 BOSS 的行為邏輯，包括狀態轉換、移動、攻擊、受傷反應等
    def update(self, player_x, player_y, bullets, enemies, enemy_bullets):
        self.state_timer += 1
        player_pos = pygame.math.Vector2(player_x, player_y)
        ...
        self.rect.center = (round(self.x), round(self.y))
        self.emit_attacks(enemy_bullets)
    # 在追蹤階段和某些攻擊前會讓 BOSS 面向玩家當前位置，這樣玩家需要不斷移動來避免被鎖定並預測 BOSS 的攻擊方向
    ...
    # 在狂暴範圍攻擊開始時，隨機生成多個攻擊點，其中一個會鎖定在玩家當前位置附近，並根據玩家位置計算衝刺方向，這樣玩家需要同時應對多個威脅並預判 BOSS 的行動
    ...
    # 在狂暴衝刺攻擊開始時，根據玩家當前位置計算衝刺方向，並在衝刺過程中保持這個方向不變，這樣玩家需要預判 BOSS 的衝刺路徑並提前移動以躲避攻擊
    ...
    # 在第一階段核心被擊敗後觸發，將 BOSS 轉換到第二階段的狀態，重置 HP 為 1 並開始變身過程，這樣玩家需要在變身過程中保持距離並等待變身完成後才能繼續攻擊
    ...
    # 在 BOSS 受到致命傷害時，如果尚未進入第二階段，則觸發變身過程並讓 BOSS 存活下來，否則正常死亡，這樣玩家需要先擊破第一階段核心才能真正擊敗 BOSS
    def survive_lethal_damage(self):
        if not self.phase2_started: self.start_phase_two(); return True
        return False
    # 根據當前狀態判斷 BOSS 是否可以受到傷害，通常在進入某些攻擊或變身狀態時會暫時無敵，玩家需要抓住這些時機進行攻擊
    def can_take_damage(self): return self.state not in ("ENTRANCE", "TRANSFORM", "DEFEAT")
    # 根據當前狀態和階段，發出對玩家造成威脅的攻擊模式，包括追蹤性攻擊、範圍攻擊、衝刺攻擊等，並在適當的時機播放攻擊音效
    def emit_attacks(self, enemy_bullets):
    ...
    # 根據當前狀態和階段繪製 BOSS 的外觀，包括身體、攻擊預兆、特殊效果等，並根據狀態變化呈現不同的視覺反饋
    def draw(self, surface):
        cx, cy = (round(self.x - camera_x), round(self.y - camera_y))
        ...
        if anim_frames and self.state != "DEFEAT":
            ...
            if self.state == "DEFEAT":
                ...
                pygame.draw.polygon(surface, core_color, [(int(p.x), int(p.y)) for p in inner])
...
# 進入 BUNKER 模式，重置遊戲狀態並根據是否成功完成突襲來處理獎勵和懲罰，在進入 BUNKER 後有一個全新的狀態來面對新的挑戰
def enter_bunker(success=False):
    ...
    task_system = TaskSystem()
# 開始突襲模式，重置遊戲狀態並生成新的挑戰，包括敵人、寶箱、抽取點等，並開始計時
def start_raid():
...
while running:
    ...
    m_x = int((raw_mx - offset_x) / scale_factor)
    m_y = int((raw_my - offset_y) / scale_factor)
    m_pos = (m_x, m_y)
    hovered_slot_info = None 

    for event in pygame.event.get():
        # 攔截所有滑鼠事件，將座標無縫轉換成虛擬座標
        # 這樣程式碼底下所有的 event.pos 就不用一個一個改可以自動適應
        if event.type in (pygame.MOUSEBUTTONDOWN, pygame.MOUSEBUTTONUP, pygame.MOUSEMOTION):
            e_dict = event.dict.copy()
            e_dict['pos'] = (int((event.pos[0] - offset_x) / scale_factor), int((event.pos[1] - offset_y) / scale_factor))
            event = pygame.event.Event(event.type, e_dict)
            
        if event.type == pygame.QUIT: running = False
        if game_state == "DIALOGUE":
            if (event.type == pygame.MOUSEBUTTONDOWN and event.button == 1) or \
               (event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE):
                dialogue_sys.next_line()
            continue  # 對話中攔截其他所有操作，不讓玩家開槍或走動
        ...
        # 鼠標事件處理，包含背包拖放、黑市購買、收藏箱管理、改造台選擇、武器庫裝備等
        if show_inventory and game_state == "PLAYING":
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                drag_data = None
         # 收藏箱界面，左鍵拖放物品，右鍵快速轉移，點擊空白處丟棄物品，點擊關閉按鈕返回地堡       
        elif game_state == "GENERAL_STASH":
            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1:
                    for i in range(36):
    ...
    if game_state == "BUNKER":
        ...
        # 超時瞬間觸發對話 ，只觸發一次
        if extraction_timer <= 0 and not boss_army_active:
            boss_army_active = True
            dialogue_sys.start("timeout_warning", "PLAYING") # 觸發超時劇情
        ...
        if player.level % 5 == 0 and player.level > 0 and player.level not in defeated_boss_levels and not boss_active and not boss_army_active:
            ...
            if boss_cycle == 0: 
                ...
                dialogue_sys.start("boss1_intro", "PLAYING") # 觸發 Boss 1 劇情
            elif boss_cycle == 1: 
                boss = ChargerBoss(player.level, player.x, player.y)
                dialogue_sys.start("boss2_intro", "PLAYING") # 觸發 Boss 2 劇情
            else: 
                boss = BerserkerBoss(player.level, player.x, player.y)
                dialogue_sys.start("boss3_intro", "PLAYING") # 觸發 Boss 3 劇情
            
            boss_active = True
            pygame.mixer.music.pause()       # 暫停普通背景音樂
            play_sound("boss_bgm", loop=-1)  # 播放 Boss 戰鬥專屬 BGM
            ...
            if game_mode == "CHALLENGE":
                if player.ammo <= 0: 
                    ...
                    if player.reload_timer <= 0:
                        ...
                        play_sound("reload")  # 開始換彈時播放音效
                    else:
                        play_sound("empty_click")  # 狂按滑鼠會發出卡彈聲(有內建冷卻不吵)
                else: 
                    player.ammo -= 1
                    if player.ammo <= 0:
                        player.reload_timer = player.reload_duration
                        play_sound("reload")  # 打完最後一發自動換彈音效
        ...
        if boss_active and boss:
            ...
            if boss.state == "DEFEAT" and boss.defeat_timer > 60:
                ...
                stop_sound("boss_bgm")           # 停止 Boss 音樂
                pygame.mixer.music.unpause()     # 恢復普通背景音樂
        ...
        if player.hp <= 0 and game_state == "PLAYING":
            ...
            play_sound("gameover")
            stop_sound("boss_bgm")
            pygame.mixer.music.unpause()  # 死亡畫面恢復普通音樂
        ...
        if game_state == "PLAYING":
            ...
            for g in items[:]:
                ...
                if player.rect.colliderect(g.rect):
                    if g.item_type in ["EXP", "MAGNET", "BOMB", "SHIELD"]:
                        items.remove(g)
    ...
    # 畫面繪製
    if game_state in ["BUNKER", "SHOP", "GENERAL_STASH", "MOD_STATION", "WEAPON_STASH"] or (game_state == "DIALOGUE" and dialogue_sys.previous_state == "BUNKER"):
        screen.fill(BLACK)
        ...
        # 戰鬥與暫停 UI 動態切換：在遊戲進行中，隱藏部分 UI 元素以減少干擾在暫停或升級界面提供完整的資訊展示，讓玩家能夠專注於當前的遊戲狀態，同時在需要時獲得必要的資訊和選項
        if game_state == "PLAYING":
            # 戰鬥中隱藏右側龐大的卡牌列表，並將任務改為迷你版放在小地圖下方
            draw_task_panel(screen, task_system, WIDTH - 190, 160, compact=True)
        else:
            # 只有當玩家按 ESC 暫停、升級或死亡時，才展開完整的面板
    ...
    if game_state == "MENU":
        ...
        controls = ["移動: WASD", "射擊: 左鍵  |  技能: 右鍵  |  衝刺: Q", "互動: E  |  開箱: F", "切換武器: E  |  背包: TAB  |  補血: H"]
    ...
    if game_state == "DIALOGUE":
        ...
        darken.fill((0, 0, 0, 100)) # 讓背景變暗一點
        screen.blit(darken, (0, 0))
        draw_dialogue_box(screen)
    
    # 繪製自訂射擊準心游標(目前找不到滿意的圖片)
    crosshair_img = images.get("crosshair")
    if crosshair_img:
        # 如果有圖片將圖片中心點對齊滑鼠
        cursor_rect = crosshair_img.get_rect(center=m_pos)
        screen.blit(crosshair_img, cursor_rect)
    else:
        # 如果沒有圖片會使用備用準心
        pygame.draw.line(screen, WHITE, (m_x - 12, m_y), (m_x - 4, m_y), 2)
        pygame.draw.line(screen, WHITE, (m_x + 4, m_y), (m_x + 12, m_y), 2)
        pygame.draw.line(screen, WHITE, (m_x, m_y - 12), (m_x, m_y - 4), 2)
        pygame.draw.line(screen, WHITE, (m_x, m_y + 4), (m_x, m_y + 12), 2)
        pygame.draw.circle(screen, RED, (m_x +1 , m_y +1), 3)

    # 最終畫面渲染 (Letterboxing 我不確定中文翻譯) 基本上就是計算畫面縮放與置中，無論任何解析度比例都不會壞掉變形
    win_w, win_h = real_screen.get_size()
    scale_factor = min(win_w / BASE_WIDTH, win_h / BASE_HEIGHT)
    ...
    # 將內部的 1024x768 畫布平滑縮放後畫到實際的螢幕上
    scaled_surf = pygame.transform.scale(screen, (new_w, new_h))
    
    # 清空背景並填上深灰色黑邊，再貼上遊戲畫面
    real_screen.fill((8, 10, 15)) 
