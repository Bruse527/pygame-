...
def draw_task_panel(surface, task_system, x, y):
    ...
    if task_system.current_task:
        ...
        # 任務名稱
        name_surf = font.render(task.name, True, YELLOW)
        surface.blit(name_surf, (x + 15, y + 50))
        
        # 任務描述
        desc_surf = small_font.render(task.description, True, WHITE)
        surface.blit(desc_surf, (x + 15, y + 85))
        
        # 進度條
        progress_bar_w = 280
        progress_percent = min(1.0, task.current_progress / task.objective_value) if task.objective_value > 0 else 0
        pygame.draw.rect(surface, GRAY, (x + 15, y + 120, progress_bar_w, 12), border_radius=3)
        pygame.draw.rect(surface, (0, 200, 100), (x + 15, y + 120, progress_bar_w * progress_percent, 12), border_radius=3)
        
        # 進度百分比文字
        progress_txt = f"{int(task.current_progress)}/{task.objective_value}"
        progress_surf = tiny_font.render(progress_txt, True, WHITE)
        surface.blit(progress_surf, (x + 20, y + 121))
        
        # 獎勵資訊
        reward_text = ""
        reward_color = YELLOW
        if task.reward_type == "scrap":
            reward_text = f"獎勵: {task.reward_amount} 廢料"
        elif task.reward_type == "exp":
            reward_text = f"獎勵: {task.reward_amount} 經驗"
        elif task.reward_type == "dmg_bonus":
            reward_text = f"獎勵: +{task.reward_amount} 傷害"
        elif task.reward_type == "max_hp":
            reward_text = f"獎勵: +{task.reward_amount} 血量"
        elif task.reward_type == "max_stamina":
            reward_text = f"獎勵: +{task.reward_amount} 體力"
        
        reward_surf = small_font.render(reward_text, True, reward_color)
        surface.blit(reward_surf, (x + 15, y + 150))
    else:
...
class Task:
    def __init__(self, task_id, name, description, objective_type, objective_value, reward_type, reward_amount):
        ...
        self.objective_type = objective_type  # "kill", "collect", "damage", "survive"
        self.objective_value = objective_value
        self.current_progress = 0
        self.is_completed = False
        self.reward_type = reward_type  # "scrap", "exp", "max_hp", "dmg_bonus"
...
class TaskSystem:
    def __init__(self):
        ...
        self.task_pool = [
            ...
            {"name": "金鑰匙收藏家", "desc": "收集 5 個金鑰匙", "type": "collect", "value": 5, "reward_type": "scrap", "reward": 80},
            # 戰鬥任務
            ...
            # 強化任務
            {"name": "強身計畫", "desc": "升級最大血量", "type": "upgrade", "value": 1, "reward_type": "max_hp", "reward": 20},
            {"name": "體能訓練", "desc": "升級最大體力", "type": "upgrade_stamina", "value": 1, "reward_type": "max_stamina", "reward": 20},
            # 生存任務
    ...
    def generate_new_task(self):
        if len(self.completed_tasks) < len(self.task_pool):
            available_tasks = [t for i, t in enumerate(self.task_pool) if i not in self.completed_tasks]
            if available_tasks:
                task_data = random.choice(available_tasks)
                task_id = self.task_pool.index(task_data)
                self.current_task = Task(
                    task_id, task_data["name"], task_data["desc"],
                    task_data["type"], task_data["value"],
                    task_data["reward_type"], task_data["reward"]
                )
            else:
                self.current_task = None
        else:
...
task_system = TaskSystem()  # 初始化任務系統
raid_start_time = None  # 用於追蹤突襲開始時間
...
def start_raid():
    ...
    raid_start_time = pygame.time.get_ticks()  # 記錄突襲開始時間
...
while running:
    ...
    #遊戲主循環，根據當前的遊戲狀態更新遊戲邏輯，包括玩家移動、敵人生成和行為、子彈更新、物品交互和界面交互等，確保遊戲在不同狀態下有適當的行為和反應
        ...
        # 更新生存時間任務 (每秒更新一次，約 60 幀)
        if raid_start_time and int((pygame.time.get_ticks() - raid_start_time) / 1000) != int((pygame.time.get_ticks() - raid_start_time - 16) / 1000):
            task_system.update_progress("survive", 1) 
                ...
                if g.item_type not in ["EXP", "MAGNET", "BOMB", "SHIELD"] and math.hypot(player.x - g.x, player.y - g.y) < 70:
                    ...
                    if player.add_item(new_item):
                        ...
                        if g.item_type == "SCRAP":
                            task_system.update_progress("collect", g.count)
        ...
        if task_system.current_task and task_system.current_task.is_completed:
            task_system.current_task.apply_reward(player)
            play_sound("levelup")
        ...
        for b in bullets:
            ...
            for e in enemies[:]:
                if b.rect.colliderect(e.rect):
                    ...
                    play_sound("hit")
                    # 更新任務進度 - 造成傷害
                    task_system.update_progress("damage", b.damage)
                    ...
                    if e.hp <= 0 and e in enemies:
                        ...
                        if e.is_elite:
                            task_system.update_progress("kill_elite", 1)
                        if e.is_elite: 
