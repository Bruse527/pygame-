...
class Player:
    ...
    def update(self):
        ...
        if keys[pygame.K_LCTRL] and not self.is_dashing and self.stamina >= self.dash_cost:
...
class Bullet:
    def __init__(self, x, y, target_x, target_y, is_piercing=False, is_homing=False):
    ...
    def update(self):
        if self.is_homing and enemies:
            # 尋找最近的敵人
            nearest_enemy = min(enemies, key=lambda e: self.pos.distance_to(e.pos))
            target_dir = nearest_enemy.pos - self.pos
            if target_dir.length() > 0:
                target_dir.normalize_ip()
                turn_speed=0.02
                # 緩慢轉向目標 (轉向速度 0.1)，增加追蹤的動態感
                self.direction += target_dir * turn_speed
                self.direction.normalize_ip()
        targets_in_front = [
            e for e in enemies
            if self.direction.dot(e.pos - self.pos)
        ]
        if self.is_homing and targets_in_front:
            target_enemy = min(targets_in_front, key=lambda e: self.pos.distance_to(e.pos))
...
def apply_upgrade(choice):
    ...
    elif choice == 4: player.homing_level += 1        
    game_state = "PLAYING"             
...
upgrade_options =[
    ...
    {"title": "追蹤晶片", "desc": ["子彈獲得自動", "尋敵追蹤功能"]}
]
cards =[pygame.Rect(100, 200, 160, 240), pygame.Rect(320, 200, 160, 240), pygame.Rect(540, 200, 160, 240), pygame.Rect(760, 200, 160, 240), pygame.Rect(980, 200, 160, 240)]
...
while running:
    ...
    if game_state == "PLAYING":
        ...
        if mouse_btns[0] and shoot_cooldown <= 0 and not player.is_dashing:
            ...
            bullets.append(Bullet(player.rect.centerx, player.rect.centery, mouse_x, mouse_y, is_piercing,is_homing=(player.homing_level > 0)))
